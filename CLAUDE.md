...
**Implementation:**
- `docs/guides/DEVELOPMENT_PATTERNS.md` - 94 active development patterns with examples (highest numbered slot is 96; slots 51 + 85 reserved-for-future-promotion, so 96 - 2 = 94 active). V1.47 amends Pattern 92 from 5-Axis to 6-Axis Tier-Separation Test (NEW Axis 5: cross-environment verification; origin Migration 0090 multi-session arc — Phase 4 MCP-probed dev DB only, Phase 6 hit `relation "markets_pkey" does not exist` against test DB carrying auto-named `markets_pkey1` form; build spec OQ-H1 PRESERVE→NORMALIZE flip + Step 0 idempotent pre-step the structural recovery; Axis 5 codifies the discipline against environment-conditional artifacts). V1.46 adds Pattern 96 (Comprehensive Audit-Set Discipline for Code Modifications; N=7+ cascade-wave evidence from session 104 Migration 0090 retrospective coverage closure — 8-category audit-set enumeration MUST be specified at PM dispatch time; broader-grep is backstop not substitute). V1.45 adds 4 new patterns + Pattern 91 V1.45+ scope expansion (PM dispatch-prompt-authoring tier; N=9 explicit chain-of-trust catches across sessions 94-98, conservative cumulative aggregate ≥N=11 including V1.44 sessions 89-91 carry-forward): Pattern 92 (5-axis tier-separation test for canonical-vs-platform-vs-domain-spoke schema audits; Galadriel session 98 corrective discipline), Pattern 93 (SELECT-star view recreation antipattern; session 98 Migration 0089 fix-pass catch — 14 round-trip CI failures → 0 via explicit column list), Pattern 94 (persistent test DB pollution recovery via single-step bounce), Pattern 95 (migration trigger-function whitespace round-trip fragility). V1.45 also scope-narrows Pattern 82 V2 to canonical_markets only post-Slot-2 (canonical_entities ref_team_id retired by Migration 0086). V1.44 adds Pattern 91: MCP-First Premise Verification for Authoring Artifacts (ALWAYS when an authoring artifact makes a claim about live schema state; N=3 from sessions 89+90+91 caught at three distinct pipeline stages — PM premise check / Builder execution / Phase 3 adversarial review). V1.43 adds Patterns 89 + 90: Post-Merge Sentinel Recovery + Auto-Merge State Hygiene. V1.39 adds Pattern 86: Living-Doc Freshness Markers (codifies the `<!-- FRESHNESS: ... -->` HTML-comment shape DATABASE_SCHEMA_SUMMARY V2.1 demonstrates).
...
**Database & API:**
- `docs/database/DATABASE_SCHEMA_SUMMARY_V2.4.md` - Complete schema (supersedes V2.1+; FRESHNESS marker `alembic_head=0091` post-Migration 0091 Cohort 5+ Slot B — `canonical_event_match_log` audit ledger + `canonical_events.created_by` provenance column + `match_algorithm` matcher seed row; canonical demonstration of Pattern 86 freshness markers — earlier versions retained for changelog history)
