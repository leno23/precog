...
| **MASTER_INDEX.md** | ✅ | v2.57 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | THIS FILE - complete document inventory - **UPDATED V2.57** (Cohort 1 carry-forward amendment session 75 / Issue #1011: ARCHITECTURE_DECISIONS V2.39 → V2.40 with 5 PM-adjudicated decisions — partial unique index for `canonical_participant_roles` cross-domain singleton, `sequence_number` NO DEFAULT ratified, `lifecycle_phase` CHECK + 0077 mirror, Pattern 82 V2 Forward-Only Direction Policy + load-bearing regression test, seed-subquery NULL guard → Pattern 83; ADR_INDEX V1.29 → V1.30 amended ADR-118 row; DEVELOPMENT_PATTERNS V1.36 → V1.37 Pattern 82 V2 + Pattern 83 promoted in parallel PR); V2.56 (Cohort 2 amendment session 73). Filename de-versioned per Epic #1054 Phase 1 — version lives in frontmatter + this changelog block. |
| **ARCHITECTURE_DECISIONS.md** | ✅ | v2.47 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers - **UPDATED V2.47** (Slot 5 of cleanup epic #1155 close-out, session 99 / PR #1175: Slots 1-4 ratification + R6 codification (R3+R5'+R8) + Pattern 82 V2 scope-narrowing + V2.40 Item 4 pin formal retirement + Pattern 91/92/93/94/95 forward-pointers to DEVELOPMENT_PATTERNS V1.45; epic #1155 CLOSED); V2.40 (ADR-118 Cohort 1 carry-forward amendment session 75 / Issue #1011: 5 PM-adjudicated decisions from 3-agent council — partial unique index `canonical_participant_roles (role) WHERE domain_id IS NULL`; `sequence_number` NO DEFAULT ratified; `canonical_events.lifecycle_phase` CHECK NOW + mirror on Migration 0077 `canonical_event_phase_log.phase`; Pattern 82 V2 Forward-Only Direction Policy + load-bearing regression test; seed-subquery NULL guard → Pattern 83 (V1.37); Future-Cohort `canonical_events.(game_id, series_id)` Pattern 82 finding flagged for Migration 0085); V2.39 (Cohort 2 amendment session 73). Filename de-versioned per Epic #1054 Phase 1. |
