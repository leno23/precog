...
| **ARCHITECTURE_DECISIONS.md** | ✅ | v2.40 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers - **UPDATED V2.40** (ADR-118 Cohort 1 carry-forward amendment session 75 / Issue #1011: 5 PM-adjudicated decisions from 3-agent council — partial unique index `canonical_participant_roles (role) WHERE domain_id IS NULL`; `sequence_number` NO DEFAULT ratified; `canonical_events.lifecycle_phase` CHECK NOW + mirror on Migration 0077 `canonical_event_phase_log.phase`; Pattern 82 V2 Forward-Only Direction Policy + load-bearing regression test; seed-subquery NULL guard → Pattern 83 (V1.37); Future-Cohort `canonical_events.(game_id, series_id)` Pattern 82 finding flagged for Migration 0085); supersedes V2.39 (Cohort 2 amendment session 73). Filename de-versioned per Epic #1054 Phase 1. |
