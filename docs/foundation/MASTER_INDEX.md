...
**Status:** ✅ Current
**Changes in v2.58:**
- **ADR-118 V2.49 — OQ-H1 PRESERVE → NORMALIZE FLIP CODIFICATION (session 105):**
  - ARCHITECTURE_DECISIONS V2.48 → V2.49: Encoded the session-104 commit `4d74317` OQ-H1 adjudication flip in canonical-doc form (test + runbook updates already shipped; this is the formal ADR record). 7 sub-sections § V2.49-A through § V2.49-G covering adjudication flip + commit anchor, rationale (cost asymmetry / natural fire-stage / forward-only directionality), forward effect (0 trailing-`1` sequences/constraints MCP-verified), Cohort 5+ retirement (V2.48 § V2.48-I item 5 RETIRED — completed in-slot), Pattern 92 6th-axis cross-env-verification candidate, Pattern 91 V1.45+ self-application footnote, closes session-104 task #7.
  - No new tables, no new migrations. Pattern 87 immutability preserved: V2.48 prose UNTOUCHED; V2.49 is appended at top of changelog block. Pattern 73 SSOT discipline applied: V2.49-D is the canonical retirement record for V2.48 § V2.48-I item 5 (no in-place edit of V2.48 prose).
  - Cross-references: PR [#1183](https://github.com/mutantamoeba/precog/pull/1183) (Migration 0090); commit [`4d74317`](https://github.com/mutantamoeba/precog/commit/4d74317) (test + runbook flip); session 104 close summary task #7; follow-on epic [#1184](https://github.com/mutantamoeba/precog/issues/1184) item-5 retirement.
**Changes in v2.57:**
- **ADR-118 COHORT 1 CARRY-FORWARD AMENDMENT** (Issue #1011, session 75, closes #1011):
  - ARCHITECTURE_DECISIONS V2.39 → V2.40: Encoded 5 PM-adjudicated Cohort 1 carry-forward design decisions from 3-agent council (Holden + Galadriel + Joe Chip, session 74) — partial unique index on `canonical_participant_roles (role) WHERE domain_id IS NULL`; `canonical_event_participants.sequence_number` NO DEFAULT ratified; `canonical_events.lifecycle_phase` CHECK NOW with mirror requirement on Migration 0077; **Pattern 82 V2 Forward-Only Direction Policy** + mandatory load-bearing regression test (load-bearing per ADR text — MUST NOT be skipped, retired, or admitted to any audit bypass set); seed-subquery NULL guard promoted to Pattern 83 (V1.37). Future-Cohort consideration filed: `canonical_events.(game_id, series_id)` Pattern 82 instance for Migration 0085 (Cohort 6) territory. No new tables.
  - ADR_INDEX V1.29 → V1.30: ADR-118 row amended to reference v2.40 Cohort 1 carry-forward amendment.
  - DEVELOPMENT_PATTERNS V1.36 → V1.37 (parallel PR): Pattern 82 V2 wording (Forward-Only Direction Policy with load-bearing regression test); Pattern 83 promoted (Seed-Time Foreign-Key Subquery Guard for Cross-Migration Lookups). 82 → 83 documented patterns.
  - V2.56 deleted per supersede-and-delete convention (#979 / #1009 / #1018 precedent).
**Changes in v2.56:**
- **ADR-118 COHORT 2 AMENDMENT** (session 73):
  - ARCHITECTURE_DECISIONS V2.38 → V2.39: Encoded 5 user-adjudicated Cohort 2 design decisions for `canonical_markets` (Migration 0069) — DDL audit-column shorthand expanded; `market_type_general` inline `CHECK` constraint + Pattern 81 non-application prose; `lifecycle_phase` explicitly excluded with three-distinct-concerns rationale (canonical_events.lifecycle_phase / platform markets.status / canonical_markets.retired_at); `updated_at` + `BEFORE UPDATE` trigger established as #1007 retrofit template; Pattern 82 non-application + `ON DELETE RESTRICT` ratified explicitly. Bonus footnotes for Migrations 0076 / 0078 / 0083 (Issues #1015 / #1016 / #1017). No new tables.
  - ADR_INDEX V1.28 → V1.29: ADR-118 row amended to reference v2.39 Cohort 2 amendment.
  - V2.55 deleted per supersede-and-delete convention (#979 / #1009 precedent).
**Changes in v2.55:**
- **ADR-118 COHORT 1 AMENDMENT** (session 72, closes #996):
  ...
  - DEVELOPMENT_PATTERNS V1.35 → V1.36: Promoted Pattern 81 (Open Canonical Enum → Lookup Table Over CHECK Constraint) and Pattern 82 (CONSTRAINT TRIGGER for Polymorphic Typed Back-Reference) — both operationalize ADR-118 v2.38; 4 lookup-table instances + 1 CONSTRAINT TRIGGER instance landed in Migrations 0067 + 0068 (PRs #1005 + #1008); 80 → 82 documented patterns. V1.35 deleted per supersede-and-delete convention (#979 precedent).
**Changes in v2.54:**
...
**Changes in v2.38:**
- **DATA COLLECTION GUIDE V1.1**: Updated DATA_COLLECTION_GUIDE_V1.0.md → DATA_COLLECTION_GUIDE_V1.2.md
...
| **MASTER_REQUIREMENTS_V2.27.md** | ✅ | v2.27 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | Complete requirements through Phase 10 with REQ IDs - **UPDATED V2.27** (Slot 5 of cleanup epic #1155, session 99: Section 4.19 NEW — REQ-CANON-LIFECYCLE-2 + REQ-CANON-RESOLUTION-DIVERGENCE-1 codify the R6 architectural decision shipped in Slot 4 Migrations 0088 + 0089; canonical_markets lifecycle independence from canonical_events; 5-value canonical_markets.lifecycle_phase enum + canonical_market_phase_log audit ledger; 4 stress cases codified including Bills @ Chiefs voided-total + Chicago O'Hare weather event-completes-but-market-settles-later; ADR-118 V2.47 amendment is the cross-reference home); V2.26 (Task 9 session 71: Section 4.18 LLM/MCP Integration Requirements — REQ-LLM-001..016 covering shared service layer, MCP surfaces, deployment, observability, fallback, error taxonomy, contract versioning/parity/credentials; Epic #990) |
| **MASTER_INDEX.md** | ✅ | v2.58 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | THIS FILE - complete document inventory - **UPDATED V2.58** (Platform-prefix rename design lock session 100 / PR-X: ARCHITECTURE_DECISIONS V2.47 → V2.48 amendment as design lock for Cohort 5+ Slot A platform-prefix rename — naming convention 3-prefix taxonomy codified, 7-table rename locked list, 15-column FK rename matrix, lookup-table prefix exemptions, internal-trading-state carve-out, Cohort 5+ slot ordering ratified — Migration 0090 ships separately as PR-Y); V2.57 (Cohort 1 carry-forward amendment session 75 / Issue #1011: ARCHITECTURE_DECISIONS V2.39 → V2.40 with 5 PM-adjudicated decisions); V2.56 (Cohort 2 amendment session 73). Filename de-versioned per Epic #1054 Phase 1 — version lives in frontmatter + this changelog block. |
| **ARCHITECTURE_DECISIONS.md** | ✅ | v2.49 | `/docs/foundation/` | 0 | Phases 1-10 | 🟡 High | Design rationale with ADR numbers - **UPDATED V2.49** (ADR-118 OQ-H1 PRESERVE → NORMALIZE flip codification, session 105: § V2.49-A through § V2.49-G codify the session-104 commit `4d74317` flip — Migration 0090 Step 0 trailing-`1` normalization pre-step + test invariant inversions + runbook decay closure; retires V2.48 § V2.48-I item 5 follow-on; flagged Pattern 92 6th-axis cross-env-verification candidate; Pattern 91 V1.45+ MCP-grounded "0 trailing-`1` sequences" live verification); V2.48 (Platform-prefix rename design lock for Cohort 5+ Slot A, session 100 / PR-X: 11 sub-sections § V2.48-A through § V2.48-K codifying 3-prefix taxonomy + 7-table rename locked list + 15-column FK rename matrix + lookup-table carve-outs + internal-trading-state carve-out + Pattern 87 reaffirmation + Cohort 5+ follow-on commitments; deferred-realization of ADR-118 V2.46 line 18092 deferral; Migration 0090 ships separately as PR-Y); V2.47 (Slot 5 of cleanup epic #1155 close-out, session 99 / PR #1175: Slots 1-4 ratification + R6 codification + Pattern 82 V2 scope-narrowing + V2.40 Item 4 pin formal retirement + Pattern 91/92/93/94/95 forward-pointers); V2.40 (ADR-118 Cohort 1 carry-forward amendment session 75 / Issue #1011); V2.39 (Cohort 2 amendment session 73). Filename de-versioned per Epic #1054 Phase 1. |
...
| **ADR_INDEX.md** | ✅ | v1.30 | `/docs/foundation/` | 0 | All phases | 🔴 Critical | Systematic catalog of architecture decisions - **UPDATED V1.30** (ADR-118 row amended for Cohort 1 carry-forward v2.40 amendment — Issue #1011 / session 75; supersedes V1.29 Cohort 2 v2.39 amendment session 73). Filename de-versioned per Epic #1054 Phase 1. |
...
| Document | Status | Version | Location | Phase | Phase Ties | Priority | Notes |
|----------|--------|---------|----------|-------|------------|----------|-------|
| **DATABASE_SCHEMA_SUMMARY_V1.15.md** | ✅ | v1.15 | `/docs/database/` | 0.5-2 | Phases 1-10 | 🔴 Critical | **UPDATED V1.15** - Added games table for historical game data, team_id FK columns, Migration 0013 |
...
| **DEVELOPMENT_PATTERNS.md** | ✅ | v1.38 | `/docs/guides/` | All | All phases | 🔴 Critical | **UPDATED V1.38** (session 76) - Added Pattern 84 (#1037). Prior V1.37 (session 75, closes #1011): amended Pattern 82 with V2 Forward-Only Direction Policy + added Pattern 83 (Seed-Time FK Subquery Guard for Cross-Migration Lookups) — operationalizing ADR-118 v2.40 Cohort 1 carry-forward. Prior V1.36 (session 72, closes #996): added Pattern 81 (Open Canonical Enum → Lookup Table Over CHECK Constraint) and Pattern 82 (CONSTRAINT TRIGGER for Polymorphic Typed Back-Reference) — both operationalize ADR-118 v2.38 Cohort 1 amendment; 4 lookup-table instances + 1 CONSTRAINT TRIGGER instance landed in Migrations 0067 + 0068 (PRs #1005 + #1008). Prior V1.35 (session 71): added Pattern 80 (SCD-2 Version-Stable Surrogate Identifiers) from ADR-119 Part 1; deleted per supersede-and-delete convention. Prior V1.34 (session 67): added Pattern 79 (Three-Tier Identity Model) from Epic #935. Filename de-versioned per Epic #1054 Phase 1. |
...
| **DATA_COLLECTION_GUIDE_V1.2.md** | ✅ | v1.2 | `/docs/guides/` | 3+ | Phases 3+ | 🔴 Critical | **UPDATED V1.2** - Added historical data caching infrastructure (Kalshi markets/series/positions, ESPN games); unified cache paths via cache_config.py; data source adapters (nflreadpy, pybaseball, nba_api); V1.1: Data Source Tiering Strategy |
| **ELO_COMPUTATION_GUIDE_V1.2.md** | ✅ | v1.2 | `/docs/guides/` | 2C | Phase 2C | 🔴 Critical | **NEW** - Elo rating computation guide (~400 lines): Core Elo formula with K-factor, home field advantage, margin of victory multiplier; sports-specific parameters (NFL K=20, NBA K=25, MLB K=15); batch processing of historical games; team rating storage and retrieval API; win probability calculation; ADR-109, REQ-ELO-001 through REQ-ELO-007 |
...
| **CODE_REVIEW_TEMPLATE_V1.0.md** | 📦 | v1.0 | `/_archive/` | 0.7b | All phases | 🟢 Medium | **ARCHIVED** - Superseded by agent pipeline code review process |
...
| **TDD_FAILURE_ROOT_CAUSE_ANALYSIS_V1.0.md** | 📦 | v1.0 | `/_archive/` | 1.5 | Phase 1.5 | 🟢 Medium | **ARCHIVED** - TDD failure post-mortem; lessons incorporated into testing strategy |
| **TESTING_GAPS_ANALYSIS.md** | 📦 | n/a | `/_archive/` | 1.5 | Phase 1.5 | 🟢 Medium | **ARCHIVED** - Phase 1.5 test coverage gaps; superseded by TESTING_STRATEGY_V3.9.md |
...
| **ESPN_DATA_MODEL_IMPLEMENTATION_PLAN_V1.0.md** | 📦 | v1.0 | `/_archive/` | 2 | Phase 2 | 🟢 Medium | **ARCHIVED** - ESPN data model implementation complete |
...
| **Handoff_Protocol_V1.0.md** | 📦 | v1.0 | `/_archive/all_claude_files/unzipped/` | 0 | All phases | 🔴 Critical | **ARCHIVED** - Original handoff protocol; superseded by SESSION_WORKFLOW_GUIDE_V1.2.md |
| **VERSION_HEADERS_GUIDE_V2.1.md** | 📦 | v2.1 | `/_archive/all_claude_files/unzipped/` | 0 | All phases | 🟡 High | **ARCHIVED** - Version control standards (superseded by inline documentation in DEVELOPMENT_PATTERNS) |
| **PHASE_0.7_DEFERRED_TASKS_V1.4.md** | 📦 | v1.4 | `/_archive/` | 0.7 | Phase 0.7 | 🟡 High | **ARCHIVED** - Phase 0.7 deferred tasks complete; all 8/8 tasks done (DEF-001 through DEF-008: pre-commit, pre-push, branch protection, schema validation) |
...
| Document | Status | Version | Location | Phase | Phase Ties | Priority | Notes |
|----------|--------|---------|----------|-------|------------|----------|-------|
| **DATA_SOURCES_SPECIFICATION_V1.0.md** | ✅ | v1.0 | `/docs/supplementary/` | 2C | Phase 2C | 🔴 Critical | **NEW** - Data source specification (~500 lines): Unified adapter pattern for historical data sources (FiveThirtyEight, ESPN, nflreadpy, pybaseball, nba_api); capability matrix (games, odds, elo); TypedDict records; caching infrastructure; cross-references ADR-106, DATA_COLLECTION_GUIDE_V1.2 |
