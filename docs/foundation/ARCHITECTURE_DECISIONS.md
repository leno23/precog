...
**Status:** ✅ Current
**Supersedes:** V2.48 (preserved in-doc per amendment-text immutability + Pattern 87 append-only discipline; supersede-and-delete convention applies to file rename when version-number is in the filename — ADR file has stable name)
**Session:** 105
**Changes in v2.49:**
- **ADR-118 OQ-H1 PRESERVE → NORMALIZE FLIP — CODIFICATION (session 104, retroactively recorded session 105).** V2.49 is a docs-only amendment that codifies the OQ-H1 adjudication flip executed inside Migration 0090 (PR [#1183](https://github.com/mutantamoeba/precog/pull/1183), merged 2026-05-13). The original V2.48 § V2.48-I item 5 + V2.48 § V2.48-J Pattern 91 footnote both adjudicated OQ-H1 (the `markets_id_seq1` / `markets_*_fkey1` / `markets_*_not_null1` trailing-`1` cruft inherited from pre-arc DB activity) as **PRESERVE** — defer the cosmetic cleanup to a future Cohort 5+ slot and let the rename produce `platform_markets_id_seq1` (with trailing-`1` intact). **Session 103 + 104 flipped the adjudication to NORMALIZE** under direct user direction (forward-only Variant B): a Step-0 idempotent normalization pre-step inside Migration 0090 itself, run BEFORE the platform-prefix renames, scrubs the trailing-`1` artifact so the post-rename schema lands clean (`platform_markets_id_seq`, `platform_markets_*_fkey`, etc.). No new tables, no new migrations ship under V2.49.

  - **§ V2.49-A — Adjudication flip + commit anchor.** OQ-H1 reverdict: **PRESERVE → NORMALIZE.** Implementation anchor: Migration 0090 Step 0 DO/EXCEPTION block (forward-only normalization pre-step). Test-side codification anchor: session 104 commit [`4d74317`](https://github.com/mutantamoeba/precog/commit/4d74317) (`fix(canonical): OQ-H1 PRESERVE -> NORMALIZE codification + runbook decay`) — inverted the trailing-`1` invariants encoded in `tests/integration/database/test_migration_0090_platform_prefix_rename.py` (FK parametrize list `_fkey1` → `_fkey`; sequence parametrize list `_seq1` → `_seq`; renamed `..._trailing_one_preserved` → `..._trailing_one_normalized`; module-docstring J5 description PRESERVE → NORMALIZE); fixed a cascade decay in `tests/property/test_database_crud_properties.py:1053` docstring example error message (legacy `platform_markets_platform_id_fkey1` → post-normalize `platform_markets_platform_id_fkey`); cleared Joe Chip P2 runbook decay finding in `docs/operations/canonical_observations_runbook.md:177` (forward-pointer to Migration 0090 added for the `markets` → `platform_market_snapshots` projection rename, unrelated to OQ-H1 but bundled in the same fix-pass).

  - **§ V2.49-B — Rationale for the flip.** Three forces converged to make NORMALIZE the correct disposition mid-flight:
    1. **Cost asymmetry detected during Phase 6 execution.** Session 103 Phase 6 cross-environment verification found drift: dev DB had clean `markets_pkey` but trailing-`1` on the other 8 markets-table objects; test DB had trailing-`1` on all 9. Preserving the cruft would have required encoding it as canonical in every test assertion + every operator-facing reference + every future ADR mention — permanent cumulative drift cost. The Step-0 DO/EXCEPTION pre-step cost ~30 lines of idempotent SQL with no failure mode (each rename swallows `undefined_object` / `undefined_table` / `duplicate_object`).
    2. **The rename slot was the natural fire-stage.** Migration 0090 was already touching all 7 markets-side objects' names. Normalizing the trailing-`1` artifact inside the same single-transaction migration is structurally cheaper than dispatching a future Cohort 5+ "cosmetic cleanup" slot that would re-touch the same surface for one bytes-removed-per-name delta. Pattern 87's "don't fix unrelated issues in the same slot" carve-out is the conservative default; here the unrelated issue was being structurally co-located with the in-scope rename and resolving it dropped a future slot from the backlog.
    3. **Forward-only directionality preserves Pattern 87 atomicity.** Variant B (forward-only, no downgrade restore of trailing-`1`) means Migration 0090 upgrade resolves the cruft + Migration 0090 downgrade leaves the names clean. Variant A (perfectly-reversible) was rejected because reintroducing trailing-`1` on downgrade is itself the kind of "fix an unrelated issue" Pattern 87 cautions against — Variant B is strictly forward progress.

  - **§ V2.49-C — Forward effect on schema.** Post-Migration-0090 public schema has **0 trailing-`1` sequence names** and **0 trailing-`1` `platform_*` constraint names**. Verified live via `mcp__postgres-dev__query` at session 105 amendment authoring time (Pattern 91 V1.45+ self-applied):
    - `SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = 'public' AND sequence_name LIKE '%1'` → 0 rows.
    - `SELECT conname FROM pg_constraint WHERE conname LIKE 'platform_%1'` → 0 rows.
    - The 7 platform sequences (`platform_events_id_seq`, `platform_markets_id_seq`, `platform_series_id_seq`, `platform_market_snapshots_id_seq`, `platform_market_trades_id_seq`, `platform_orderbook_snapshots_id_seq`, `platform_settlements_settlement_id_seq`) all have clean names.

  - **§ V2.49-D — Cohort 5+ follow-on retirement.** Item 5 of V2.48 § V2.48-I ("`markets_id_seq1` trailing `1` cleanup (per OQ-H1) — low-priority cosmetic preserved verbatim through rename") is **RETIRED — completed in-slot**. The session-103 Cohort 5+ epic **skeleton** (a pre-staged memory artifact at `memory/cohort_5_plus_followon_epic_body_skeleton.md`) had originally listed OQ-H1 as a deferred item; that skeleton item was removed during epic-body authoring **before** the epic was filed in session 105. The filed epic [#1184](https://github.com/mutantamoeba/precog/issues/1184) was therefore opened with **6 items** and does NOT include OQ-H1 — see the epic body's "Session 104 update" + "Non-goals" stanzas, which explicitly call out the in-slot retirement. The retirement transition traversed `skeleton (with OQ-H1) → authoring step (OQ-H1 removed) → filed epic (6 items, no OQ-H1)`, never `filed epic with 7 items → filed epic with 6`. Pattern 87 immutability is preserved: V2.48 § V2.48-I item 5 prose is NOT edited in-place; this V2.49-D pointer is the canonical retirement record per Pattern 73 SSOT discipline.

  - **§ V2.49-E — Pattern 92 6th-axis candidate (cross-environment verification).** Session 103 discovered the trailing-`1` cross-env drift (dev clean, test cruft) only because Phase 6 ran the migration against multiple environments. Pattern 92 (5-axis tier-separation test) currently has Axes 0-4 covering existence / FK column matrix / already-correctly-named / domain-spoke discrimination / value-enum probing. A 6th axis (cross-environment artifact-name parity check) is a promotion candidate flagged by session 103+104+105 evidence; promotion deferred to a future DEVELOPMENT_PATTERNS revision pending an N≥2 corpus.

  - **§ V2.49-F — Pattern 91 V1.45+ self-application during this amendment.** All schema state claims in V2.49 above are MCP-grounded at authoring time (queries shown in § V2.49-C). The "0 trailing-`1`" claim is the live invariant, not a memory-derived assertion. This amendment itself counts as an authoring artifact in the Pattern 91 corpus.

  - **§ V2.49-G — Closes session 104 task #7.** Per `memory/session_104_close_summary.md` task ledger row "OQ-H1 PRESERVE → NORMALIZE codification (task #7)" + session 105 next-session task list "Build spec OQ-H1 flip codification in ADR V2.48 — formal doc update beyond test + runbook (which session 104 did)". The tests + runbook flip shipped session 104 via commit `4d74317`; this V2.49 amendment is the canonical-doc record that closes the codification arc.
...
- **ADR-118 CLEANUP EPIC #1155 CLOSE-OUT: Slots 1-4 ratification + R6 codification + Pattern 82 V2 scope-narrowing + V2.40 Item 4 pin retirement (session 99).** V2.47 is the docs-only codification slot that closes the 5-slot cleanup epic #1155 (Slots 1-4 shipped Migrations 0085-0089 across sessions 95-98; Slot 5 is this amendment). The amendment ratifies the architectural decisions made in Slots 1-4, formally retires the V2.40 Item 4 pin (Pattern 82 V2 load-bearing test pin), and codifies the R6 = R3 + R5' + R8 lifecycle redistribution that Galadriel's session 98 council calibration recovered after the session 94 D2 duplicate-verdict TIER-CONFUSION FABRICATION. **No new tables, no new migrations** ship under V2.47 itself. Closes Epic #1155.
  ...
  - **§ V2.47-A — Slot 1 (Migration 0085, session 95, PR #1162) naming bundle ratification.** Codifies the locked CL-4 + CL-5 + CL-6 + Q10 decisions:
    - `canonical_entity` TABLE → `canonical_entities` (singular outlier → plural; aligns with the rest of the canonical-tier naming convention `canonical_<things>` plural).
    - `canonical_events.domain_id` → `canonical_events.event_domain_id` (FK naming rule: column-name disambiguates which `id` is being referenced; mirrors `event_type_id`).
    - `canonical_events.entities_sorted` → `canonical_events.participants_sorted` (participants is the active vocabulary post-V2.38 Cohort 1 ratification; entities is reserved for the parent table).
    - `canonical_event_participants.entity_id` → `canonical_event_participants.canonical_entity_id` (full canonical-tier prefix for cross-table FK references).
    All four renames ship as atomic ALTER TABLE / ALTER COLUMN statements in Migration 0085. Pattern 87 (Append-Only Migration Files) applies: post-merge 0085 is immutable. Reference: PR #1162; build spec at `memory/build_spec_slot_1_naming_bundle_pm_memo.md`.

  - **§ V2.47-B — Slot 2 (Migration 0086, session 96, PR #1164) FK direction flip + denorm collapse + Pattern 82 V2 scope-narrowing.**
    - **CL-1 (FK direction):** dim / source tables now point UP at canonical, never widen canonical with `ref_*` columns. Migration 0086 ADDed `teams.canonical_entity_id BIGINT NULL FK -> canonical_entities(id) ON DELETE SET NULL`. The polarity preserves the canonical-outlives-platform contract (a `canonical_entities` row is not deleted when its `teams` parent is dropped; the FK NULLs out).
    - **Pattern 84 by-analogy 3rd use (precedent-style framing).** The teams.canonical_entity_id FK landed on a populated parent table (`teams`); Pattern 84 V1.42 (NOT VALID + VALIDATE for FK on populated tables) is the canonical discipline. Slot 2 applied Pattern 84 *by analogy* — there was no canonical "NOT VALID + VALIDATE on FK ADD" Pattern at the time of slot 0080 (the 2nd use) so Slot 2 inherited slot 0080's by-analogy framing as the 3rd precedent. The "precedent-style framing" naming reflects that Pattern 84's NOT VALID + VALIDATE discipline applies whenever a populated table gains an FK constraint, regardless of whether the column being added points at canonical-tier or sport-tier or platform-tier.
    - **CL-2 (denorm collapse):** Migration 0086 DROPped `canonical_events.game_id` AND `canonical_events.series_id`. With CL-1's FK direction reversed, canonical_events no longer needs platform-side dim FKs as denormalized cache columns; readers can reach platform-side rows via `canonical_event_links.platform_event_id` → `games` or via `canonical_market_links.platform_market_id` → `markets` → `series`. The canonical-events row stays canonical-tier-pure.
    - **Pattern 82 V2 scope-narrowing.** Pattern 82 V1 (Migration 0061) applied a CONSTRAINT TRIGGER to canonical_entities (`trg_canonical_entity_team_backref`: "entity_kind=team requires ref_team_id NOT NULL"). With CL-1's FK direction flipped, the `ref_team_id` column on canonical_entities is no longer needed; Migration 0086 DROPped the column, the trigger, and the trigger function. Pattern 82 V2 (scope-narrowed) now applies to **canonical_markets only**. The canonical_entities variant is retired; the load-bearing test (`tests/database/test_canonical_entity_polymorphic_invariants.py`) was deleted at Slot 2 (Path A per session 96 PM adjudication). Pattern 82 V1 historical-reference subsection in DEVELOPMENT_PATTERNS V1.45 preserves the migration-archaeology view (V1 instance was correct under pre-Slot-2 FK direction; replay 0061 → 0086 shows V1 apply + retire in order).
    - Reference: PR #1164; build spec at `memory/build_spec_slot_2_fk_direction_pm_memo.md`.

  - **§ V2.47-C — Slot 3 (Migration 0087, session 97, PR #1167) retirement cascade + SSOT helper + Pattern 73 cycle-prevention.**
    - **CL-7 (retirement cascade via self-FK):** Migration 0087 ADDed `canonical_events.superseded_by BIGINT NULL` with self-referencing FK `-> canonical_events(id) ON DELETE SET NULL`. The column carries either NULL (for active rows + terminal tombstones) or a pointer to a successor canonical_events row (for superseded rows). Combined with the pre-existing `canonical_events.retired_at TIMESTAMPTZ NULL` column, the schema now encodes 3 row states: **active** (`superseded_by IS NULL AND retired_at IS NULL`), **terminal tombstone** (`superseded_by IS NULL AND retired_at IS NOT NULL`), and **superseded** (`superseded_by = <new_id> AND retired_at IS NOT NULL`).
    - **3-layer cycle defense (Q2 user-amplified adjudication, session 97):** the original spec carried a 2-layer defense (DB-level CHECK + helper read-side 100-hop guard). User Q2 amplification mid-session surfaced "what about production?" — at populated scale, a 100-hop guard at read time is too late if cycles get written. Spec amended to **3 layers**:
      1. **DB-level CHECK** (`canonical_events_no_self_supersession CHECK (superseded_by IS NULL OR superseded_by <> id)`) — blocks single-row id→id self-cycles. Cheap forward-looking insurance.
      2. **Helper write-side cycle check** (`crud_canonical_events.retire_canonical_event(id, superseded_by_id=...)`): before writing the supersession link, walks the chain from `superseded_by_id` and raises `ValueError` if the canonical_event being retired appears anywhere in that chain. **Pattern 73 SSOT applied to cycle prevention** — one write surface (`retire_canonical_event`) is also the one validation surface. The meaningful production-cycle barrier when the table is populated.
      3. **Helper read-side 100-hop guard** (`get_active_canonical_event`): the recursive CTE chain-walker caps at 100 hops and raises `RuntimeError` on suspected cycles; runtime safety net against any chain that escaped (1)+(2) via direct SQL writes.
    - **SSOT helper:** Migration 0087 + accompanying CRUD code shipped `crud_canonical_events.get_active_canonical_event(id)` — the canonical entry point for active-row resolution. Walks the `superseded_by` chain forward via recursive CTE and returns either the terminal-active row or `None` (tombstones return `None`, not the tombstone row, per Q1 design adjudication). Forward callers (Cohort 5+ matcher slot, future strategy/model code) MUST use the helper rather than inline `WHERE` clauses against `superseded_by` or `retired_at`. **Pattern 73 SSOT for retirement-chain reads.**
    - **Tier 1 Momentum fix-pass:** Ripley P2-1 layer-(b) coverage gap closed pre-merge via integration tests I8/I9/I10 against real DB exercising `_retirement_chain_includes`. Three-layer defense fully behaviorally tested.
    - Reference: PR #1167; build spec at `memory/build_spec_slot_3_retirement_cascade_pm_memo.md`.

  - **§ V2.47-D — Slot 4 (Migrations 0088 + 0089, session 98, PR #1173) R6 lifecycle redistribution + sport-tier denorm cleanup.** R6 = R3 + R5' + R8 per Galadriel's session 98 calibration (TIER-CONFUSION FABRICATION recovery + 5-axis test upgrade). *Note on `R8` label disambiguation:* `design_review_lifecycle_phase_galadriel_memo.md` § 1 used `R8` to label a separately-considered-and-rejected option (drop `canonical_events.lifecycle_phase` entirely; derive from `canonical_markets` aggregation). The V2.47-D `R8` label below refers to a distinct concept — canonical_events enum reduction 8→5 — introduced in the build spec for ergonomic three-sub-clause labeling under R6. The labels collide accidentally; readers running grep for `R8` will see both meanings. Three sub-clauses:

    - **R3 (canonical-tier capability close — Migration 0088).**
      - ADDed `canonical_markets.lifecycle_phase VARCHAR NOT NULL DEFAULT 'open'` with CHECK constraint enumerating the **5-value enum**: `open` / `suspended` / `settling` / `resolved` / `voided`. (Verified live via MCP at session 99: column exists; CHECK constraint `canonical_markets_lifecycle_phase_check` enumerates exactly these 5 values.)
      - ADDed `canonical_market_phase_log` table (sibling of `canonical_event_phase_log` from Migration 0079). Append-only audit ledger; trigger-driven auto-population from BEFORE UPDATE on `canonical_markets.lifecycle_phase`. Columns: `id BIGSERIAL PK`, `canonical_market_id BIGINT NOT NULL FK -> canonical_markets(id)`, `transition_at TIMESTAMPTZ NOT NULL DEFAULT now()`, `previous_phase VARCHAR NOT NULL`, `new_phase VARCHAR NOT NULL`, `changed_by VARCHAR NOT NULL`, `note TEXT NULL`. (Verified live via MCP at session 99: table exists.)
      - **V2.39 revisit-trigger marked FIRED with rationale.** V2.39 carried an explicit revisit-trigger anticipating the multi-canonical-market resolution-divergence case ("if a canonical event has N markets and M < N of them resolve divergently, where does that go?"). R3 codifies the answer: per-market resolution-state lives on `canonical_markets.lifecycle_phase`, independently of `canonical_events.lifecycle_phase`. The trigger fired at session 94 D2 (mis-diagnosed as duplicate) and recovered at session 98 (R3 = the architecturally complete answer).
      - **Pattern 73 SSOT** for the 5-value market-phase enum: `src/precog/database/constants.py:CANONICAL_MARKET_LIFECYCLE_PHASES` is the canonical home. The CHECK constraint in Migration 0088 and the Python constants tuple MUST agree; Pattern 73 audit triggers (S81 + #929 check) verify periodically.

    - **R5' (sport-tier denorm cleanup — Migration 0089).**
      - DROPped `game_states.game_status` column. Per session 98 input memo § 0 probe 7: 89.5% of historical rows carried `game_status` on `game_states` vacuously (only 542 of 5,195 games had non-NULL values; the column was a per-tick echo of the authoritative `games.game_status`). (Verified live via MCP at session 99: column absent from `game_states`.)
      - **Reader sweep.** `derive_game_status()` SSOT reconstructor (in `crud_game_states.py` or sibling) provides the transition-history reconstruction via period + clock_seconds adjacency for the narrow query class that previously read `game_states.game_status` directly.
      - **`current_game_states` view recreated with explicit 25-column list (NOT `SELECT *`).** Pattern 93 (DEVELOPMENT_PATTERNS V1.45) catch from Ripley P0-1 session 98 fix-pass: the original `SELECT *` recreation in Migration 0089 captured `canonical_event_id` (added by Migration 0080) into the view's frozen column list, creating a hidden round-trip dependency that caused 14 round-trip CI gate failures in revisions 0067-0080's downgrade path. The fix replaces `SELECT *` with the explicit 25-column list (excluding `canonical_event_id`) on both upgrade Step 3 and downgrade Step 3 of Migration 0089. Empirical result: 14 round-trip failures → 0.
      - **Note forward-looking correctness gaps tracked at #1174** (P1 #1 Rule 3 over-broad coverage; P1 #2 suspended-status propagation; P2 #3 nullable game_id INNER JOIN; P2 #4 `_GAME_STATUS_VALUES` enforcement gap). All four findings are forward-looking matcher-slot work that fires when `games.canonical_event_id` is populated by the matcher (~session 101). Slot 4 ship is correct against current production state (canonical-layer 0-row); the findings are *prerequisite-hardening* for matcher slot prep, NOT bugs against current production. Per session-99 user adjudication: all four deferred to matcher slot bundle.

    - **R8 (canonical_events enum reduction — Migration 0088).**
      - REDUCED `canonical_events.lifecycle_phase` CHECK constraint from **8 values to 5 values**:
        - **Pre-R8 (V2.39 → V2.46):** `proposed` / `listed` / `pre_event` / `live` / `suspended` / `settling` / `resolved` / `voided` (8 values; conflated event-relevance with per-market-relevance).
        - **Post-R8 (V2.47):** `proposed` / `listed` / `pre_event` / `live` / `completed` (5 values; pure event-relevance vocabulary).
      - The retired values map: `suspended` / `settling` / `resolved` / `voided` move to `canonical_markets.lifecycle_phase` (per-market); `completed` is the new terminal for event-row (the event happened). For event-row-level voiding (rare; e.g., game cancelled entirely), use `canonical_events.retired_at` (Slot 3 retirement cascade) — `retired_at IS NOT NULL` is the structural void-the-event-itself signal, not a `lifecycle_phase` value. (Verified live via MCP at session 99: the CHECK enumeration on `canonical_events.lifecycle_phase` enumerates the 5 R8 values.)

    - **Three-distinct-concerns model upgraded to four-distinct-concerns.** V2.39 introduced the three-distinct-concerns model (canonical_events.lifecycle_phase + platform markets.status + canonical_markets.retired_at). Galadriel session 98 § 3 + recovery memo upgrades this to **four-distinct-concerns** by adding `canonical_markets.lifecycle_phase` as the per-market-resolution column. The four concerns are structurally orthogonal — no two can be derived from each other without losing fidelity. See `docs/database/CANONICAL_LAYER_RELATIONSHIPS.md` § "Four-distinct-concerns lifecycle model" for the full traversal table + event-vs-market timing decoupling diagram (Bills @ Chiefs 3-market voided-total stress case + Chicago O'Hare weather event-completes-but-market-settles-later stress case).
    - **Pattern 92 origin (5-axis tier-separation test).** Galadriel session 98 § 6.2 self-calibration produced the upgrade to the 4-axis tier-separation test by adding Axis 0 (value-enum MCP probe) that fires BEFORE Axes 1-4 on column-level duplication claims. The 30-second MCP probe at session 94 would have killed the D2 duplicate verdict; the 4-axis test passed vacuously without inspecting the enum. Pattern 92 codifies the upgrade. See DEVELOPMENT_PATTERNS V1.45 Pattern 92 for the full rule + canonical shape + anti-recurrence test.
    - Reference: PR #1173; build spec at `memory/build_spec_slot_4_lifecycle_redistribution_pm_memo.md`; council memos at `memory/design_review_lifecycle_phase_{galadriel,holden,synthesis}_memo.md`.

  - **§ V2.47-E — V2.40 Item 4 pin formal retirement.** ADR-118 V2.40 Item 4 (PR #1011, session 75) introduced the "Pattern 82 V2 Forward-Only Direction Policy + mandatory load-bearing regression test" pin. The pin made the load-bearing test (`tests/database/test_canonical_entity_polymorphic_invariants.py`) explicitly NOT-skippable, NOT-retirable, and NOT-admitted to any audit-bypass mechanism — the test was the load-bearing compensating mechanism for Pattern 82's one-direction-only contract. Migration 0086 (Slot 2 / session 96) flipped the FK direction and dropped both `canonical_entities.ref_team_id` and the polymorphic enforcement trigger; the test became inapplicable (it asserted properties of a column that no longer exists) and was deleted (Path A per session 96 PM adjudication). The V2.40 Item 4 pin is hereby **formally RETIRED at V2.47** with rationale documented at the pin's original site (the V2.40 prose carries a forward-pointer reference; per Pattern 87 spirit on amendment-text immutability, the V2.40 prose itself is preserved — the retirement is codified by the V2.47-E section here). Future Pattern 82 applications (on canonical_markets per V2 scope-narrowing) carry their own load-bearing tests per the Pattern 82 V2 rule; the V2.40 pin's specific test is retired, not the discipline.

  - **§ V2.47-F — Pattern 91 V1.45+ promotion + Pattern 92 forward-pointer.** Cleanup epic Slots 1-4 + the session 98 council calibration produced N=9 explicit chain-of-trust catches across sessions 94-98 (conservative cumulative aggregate ≥N=11 including V1.44 sessions 89-91 carry-forward). The catches surface an additional verification surface (PM dispatch-prompt-authoring tier) beyond the V1.44 scope (build specs, binding-input memos, ADR amendments, design-review memos, operator runbooks). V1.45+ scope expansion is codified in DEVELOPMENT_PATTERNS V1.45 Pattern 91 § "PM dispatch prompts (V1.45+ scope expansion, session 99)" + § "Cumulative Evidence (sessions 94-98)". The dispatch-prompt-authoring surface's load-bearing rule: **Builder dispatch prompts MUST terminate at LOCAL COMMIT** — no `git push`, no `gh pr create`. The push + PR-open step is a separately-scheduled PM stage that gates on Reviewer + Sentinel + specialist-trigger findings. Bundling PR creation into the Builder workflow compresses the diff-scoped review window. The ANNOUNCE template's DESIGN REVIEW + DIFF-SCOPED REVIEW + SPECIALIST TRIGGERS surface MUST be cited in the dispatch sequencing; each trigger (S60 / S82 / T41 / C32 / C33 / etc.) is listed as FIRE or SKIP with rationale. Detail at `memory/feedback_chain_of_trust_pattern91_violation.md` + `memory/feedback_tier2_review_sequencing_dispatch_protocol.md`. **Pattern 92 (5-axis tier-separation test) forward-pointer:** DEVELOPMENT_PATTERNS V1.45 Pattern 92 codifies the upgrade Galadriel proposed at session 98; this is the architectural-frame mechanics complement to Pattern 91's chain-of-trust discipline. Both are durable institutional outputs of session 98.

  - **§ V2.47-G — Cleanup epic close-out summary.** All 5 slots of cleanup epic #1155, with locked decisions traced:

    | Slot | Session | Migration(s) | PR | ADR section | Locked decisions |
    |---|---|---|---|---|---|
    | 1 | 95 | 0085 | #1162 | § V2.47-A | CL-4 + CL-5 + CL-6 + Q10 naming bundle |
    | 2 | 96 | 0086 | #1164 | § V2.47-B | CL-1 (FK direction flip) + CL-2 (denorm collapse) + Pattern 82 V2 scope-narrowing + Pattern 84 by-analogy 3rd use |
    | 3 | 97 | 0087 | #1167 | § V2.47-C | CL-7 (retirement cascade via self-FK) + 3-layer cycle defense + Pattern 73 SSOT for cycle-prevention |
    | 4 | 98 | 0088 + 0089 | #1173 | § V2.47-D | R6 = R3 + R5' + R8 (canonical-tier capability close + sport-tier denorm cleanup + canonical_events enum reduction) |
    | 5 | 99 | none | (this PR) | § V2.47-A through § V2.47-G | docs-only codification + Pattern 91 V1.45+ + Pattern 92/93/94/95 NEW + V2.40 Item 4 pin retirement + 0077 test file deletion |

    **Convergent auto-locks (4-of-4 council agreement) carried forward from session 94 synthesis:** CL-1 (FK direction), CL-2 (drop game_id + series_id), CL-5 (event_domain_id), CL-6 (participants_sorted), CL-8 (defer execution; ADR text only — i.e., this V2.47-G summary IS the execution), CL-10 (audit doc; lives in `CANONICAL_LAYER_RELATIONSHIPS.md`), CL-11 (ADR clarification on `'espn'` vs `'espn_poller'` historical artifact per D3 council adjudication; lives in this V2.47 § N1-equivalent context).

    **Cross-references for the cleanup epic arc:**
    - Session 93 synthesis memo (`memory/session_93_synthesis_memo.md`) — origin queue (CL-1..CL-11 full text)
    - Session 94 council memos (`memory/design_review_canonical_cleanup_{galadriel,holden,miles,uhura}_memo.md`) — 4-agent paired-cluster verdicts
    - Session 94 synthesis (`memory/design_review_canonical_cleanup_synthesis.md`) — load-bearing for Slot 1+2+3+4 build-spec inheritance
    - Build specs: `memory/build_spec_slot_{1_naming_bundle,2_fk_direction,3_retirement_cascade,4_lifecycle_redistribution,5}_pm_memo.md`
    - Session 98 lifecycle_phase council memos (`memory/design_review_lifecycle_phase_{galadriel,holden,synthesis}_memo.md`) — R6 recovery + Pattern 92 origin
    - Antipattern memos (Patterns 93/94/95 promotion sources): `memory/feedback_select_star_view_recreation_antipattern.md`, `memory/feedback_persistent_test_db_state_pollution_hung_pytest.md`, slot 0076 P2 precedent (Pattern 95)
    - Forward-pointer: rename slot (~session 100) ships V2.48 ADR amendment + Migration 0090 platform-prefix rename; matcher slot (~session 101) inherits the cleanup-epic-locked canonical schema and resolves #1168 + #1174 forward-looking findings.

  - **Cross-references:** ADR-118 V2.39 (revisit-trigger origin — fired and codified at V2.47-D); ADR-118 V2.40 (Item 4 pin formally retired per V2.47-E); ADR-118 V2.42 sub-amendment B (Migration 0077 FK polarity — superseded by V2.47-B Slot 2 denorm collapse; test file deleted at Slot 5); ADR-118 V2.45 (Cohort 4 informal close — `canonical_observations` + `temporal_alignment` + `canonical_observation_event_links` foundation; preserved unchanged by V2.47); ADR-118 V2.46 (Items 2/3/5/7/9/10 codification — preserved unchanged by V2.47; cleanup epic doesn't touch the `canonical_observations` ingestion-gate or kind-vocabulary surfaces); `docs/database/CANONICAL_LAYER_RELATIONSHIPS.md` (companion doc; four-distinct-concerns model + event-vs-market timing decoupling diagram lives there); `docs/foundation/MASTER_REQUIREMENTS_V2.27.md` (new REQ-CANON-LIFECYCLE-2 + REQ-CANON-RESOLUTION-DIVERGENCE-1 cross-reference back here); `docs/guides/DEVELOPMENT_PATTERNS.md` V1.45 (Pattern 91 V1.45+ scope expansion + Pattern 82 V2 scope-narrowing edit + Pattern 92 + Pattern 93 + Pattern 94 + Pattern 95).

  - **Slot-ordering constraint:** V2.47 ships ADR amendment text + MASTER_REQUIREMENTS V2.27 + DEVELOPMENT_PATTERNS V1.45 + CANONICAL_LAYER_RELATIONSHIPS.md expansion + DATABASE_SCHEMA_SUMMARY V2.4 freshness-marker (no-op; already at alembic_head=0089) + Migration 0077 test file deletion + ~12 #1163 cosmetic items folding in a single docs-only PR. **No new migrations** ship under V2.47. Cleanup epic #1155 closes on this PR's merge. **Sessions 99-100** picks up the rename slot (V2.48 + Migration 0090 platform-prefix rename); **~Session 101** picks up matcher Slot A (re-spec'd against the post-cleanup canonical schema; first slot to actually populate `canonical_entities` + `canonical_events` from pollers; resolves #1168 + #1174 forward-looking findings as prerequisite-hardening).

**Changes in v2.46:**
- **ADR-118 CANONICAL LAYER ITEMS 2/3/5/7/9/10 CODIFICATION + COMPANION DOC (Issue #1143, session 91):** Codifies the session 91 V2.46 design-review pipeline (Galadriel + Holden 2-agent council CLEAR-WITH-AMENDMENTS + PM synthesis + multi-turn user adjudication). Items 1/4/6/8 shipped in V2.45 + Migration 0084; Items 2/3/5/7/9/10 are the deferred architectural-framing layer that V2.45 forward-pointed at. Amendment is narrative + Pattern 73 SSOT discipline rules + cross-cutting findings codification; **no new tables, no new migrations** ship under V2.46 itself. Companion deliverable `docs/database/CANONICAL_LAYER_RELATIONSHIPS.md` lands in the same PR — the canonical "pointer + map" reference future-self uses to wrap their head around the canonical layer's relationship structure without re-running the council. Closes Issue #1143.
  - **Item 2 — `observation_kind` boundary (canonicalize as exogenous-world-signal-only).** `canonical_observations.observation_kind` values record exogenous-world signals — events, states, or readings produced by sources external to the Precog system, its operators, and its models. Internal state (own fills/positions/predictions), audit metadata (operator overrides), and derived outputs (model predictions) live in their own purpose-built tables and are NEVER `observation_kind` values. Discriminating example: `market_trades` (third-party Kalshi trade tape, Migration 0028; live-DB-grounded — 10 typed columns including `taker_side`, `yes_price`, `trade_time`) is canonical-event-targeted but **NOT** a `canonical_observations` observation_kind, because individual third-party trades are exhaust data not Item-3-resolution-relevant; `market_trades` gains canonical_event association via direct `canonical_event_id` FK at the Layer-2 level (mirrors slot 0080 pattern), keeping it queryable for ML/strategy use without inflating `canonical_observations` volume by 5-10x at projected 50-500K trades/day forward-cast. See N4 issue. Manual-overrides clarification (Galadriel 2b): operator corrections to our own match decisions stay in dedicated tables (`canonical_match_overrides` per slot 0074) regardless of `source_id` — `source_kind = 'manual'` used for *recording an external truth* (operator transcribes a phone call) IS observation; used for *correcting our own decisions* is internal state and never writes to `canonical_observations`. Derived-source carve-out (Galadriel 2b): derived sources (`source_kind = 'derived'` per slot 0075 vocabulary) may register as `observation_source` rows ONLY when aggregating external signals; pure-internal derivations (predictions, computed positions) do NOT register here. **Loud-fail enforcement:** adding a new value to `OBSERVATION_KIND_VALUES` (`src/precog/database/constants.py`) requires an ADR amendment (or numbered ADR sub-decision) citing the Item 2 boundary test against the new kind. ADR-discipline + code-review is the loud-fail mechanism, since CHECK enumerations cannot encode the philosophy.
  - **Item 3 — `canonical_observations` is filtered, not universal sink (canonicalize the principle + the form).** `canonical_observations` holds only resolution-relevant + successfully-tagged observations. Untagged news / unrelated weather / off-topic econ / unmatched poll releases stay at the source or in raw staging tables (per-kind shapes deferred); they do NOT pollute `canonical_observations`. Operational membership form (Q1 PM adjudication, user-confirmed): *membership = the tagging pipeline asserted positive linkage to ≥1 active canonical_event with confidence ≥ T_kind, where T_kind is a per-kind threshold defined in the per-kind cohort design.* V2.46 codifies the form; per-kind T_kind values belong in per-kind cohort issues. Implicit-targeting kinds vs explicit-tagging kinds distinction (Galadriel 3, principled-asymmetry): `game_state` and `market_snapshot` have *implicit* tagging upstream of `canonical_observations` (the poller's targeting logic — only games/markets the system already cares about get fetched), so the filter sits upstream; `weather` / `poll` / `econ` / `news` need *explicit* tagging downstream of broad ingestion because their producers (NOAA, FiveThirtyEight, BLS, Reuters) emit volume that's mostly off-topic for our markets. **`linked_at` vs `ingested_at` semantic distinction (Holden 3c, settled-by-construction):** `canonical_observations.ingested_at` is the *original raw ingest time* (preserved when raw promotes to canonical); `canonical_observation_event_links.linked_at` is the *promotion time* (when the tagger asserted the canonical-event link). Backfill scenarios preserve `ingested_at` while setting fresh `linked_at` at promotion time. Reconciliation queries that need "when did we first hear it" use `ingested_at`; queries that need "when did we know what it meant" use `linked_at`. Per-kind raw-staging table shapes deferred per Q2 PM adjudication — V2.46 ratifies the principle (raw-staging exists where the source produces high-volume content with only a subset resolution-relevant; raw retention is the source's responsibility, promotion to `canonical_observations` is the tagging pipeline's responsibility), not the per-kind shapes.
  - **Item 5 — View-naming convention (canonicalize 5 categories, scope-limited to canonical-layer views).** **Scope-limit clause (user adjudication, revised from initial "grandfather" framing):** the lowercase `v_*` prefix convention applies to **canonical-layer views (Layers 3/4/4.5 in companion doc `CANONICAL_LAYER_RELATIONSHIPS.md`) only**. Operational rollups (`current_*` — `current_edges`, `current_markets`, etc.), lifecycle filters (`active_*`, `edge_lifecycle`), backtest views (`backtest_*`), and ML-feature views established pre-V2.46 are out-of-scope and retain their existing names. Forward-pointer to N5 deferred standardization issue (canonical-layer scope only initially; broader operational-view audit OUT of scope per user direction). 5 categories:
    1. **`v_<operation>` form** — canonical SQL operations (alignment, enrichment, phase-evidence-join). Use when the operation is itself the consumer-facing concept; reusable across domains. Example (Cohort 5+): `v_temporal_alignment` base view returning alignment edges + `canonical_event_id` without typed columns.
    2. **`v_<thing1>_<verb>_<thing2>` form** — fixed-shape cross-domain relationship views. Rare; require a docstring justifying *why* parameterization is fixed (e.g., "join window is fixed at game start — used by N consumers") per Galadriel 5d. Otherwise the no-view default applies. Without that gating discipline, the `_<verb>_` form accretes junk views.
    3. **`v_<thing>_enriched` form** — for "the canonical thing + its typed companions" feeds. Example (hypothetical): `v_canonical_observations_enriched` (universal + per-kind projection JOIN).
    4. **`v_<operation>_<domain>` form** (Galadriel 5c new category) — per-domain typed extension of an `<operation>` view. Examples (Cohort 5+): `v_temporal_alignment_sports` (JOINs to `market_snapshots` + `game_states` for typed sports cols), `v_temporal_alignment_weather` (when weather lands per ADR-119 Phase 1).
    5. **No view at all** — when the consumer's question is parameterized (window size varies, time anchor varies, JOIN target varies), consumers write SQL directly against `canonical_observations` + per-kind projections (Holden 5 tightening). Candidate Pattern in `DEVELOPMENT_PATTERNS.md` next sweep ("parameterized-shape views are anti-pattern").
    Tiebreaker rule (Holden): when both `v_<operation>` and `v_<thing1>_<verb>_<thing2>` apply, prefer `v_<operation>` — it is the cross-domain reusable shape.
  - **Item 6-follow-on — `v_temporal_alignment` is a name-family with per-domain extensions (RATIFY-AS-PROPOSED, no notes).** Companion to Item 6 schema redesign (V2.45 / Migration 0084). `v_temporal_alignment` (no suffix) is the base view returning alignment edges + canonical_event_id without typed columns — useful for cross-domain analytics ("how many alignments per kind today?"). `v_temporal_alignment_<domain>` per-domain typed extensions JOIN the linkage `temporal_alignment` table to the relevant per-kind projection tables for typed columns. Domains land per-cohort: sports per Cohort 5+ alongside writer rewiring (issue #1141); weather per ADR-119 Phase 1; election per Phase 3+.
  - **Item 7 — `observation_source.authoritative_for` reaffirmation (canonicalize discipline + commit to backfill + reject-as-default).** Three-part codification:
    1. **Discipline:** the `canonical_observations` writer asserts the source's `authoritative_for` JSONB array contains the row's `observation_kind`. Mismatch → reject. Slot 0075 ships the column shape; V2.46 makes the discipline binding when sources start writing.
    2. **Backfill commitment (Q8 PM adjudication, schema-truth delta D3; refined per session 91 Glokta F2 finding + user adjudication B=(a)):** all 3 existing `observation_source` rows currently carry `authoritative_for = NULL` (verified live via MCP — `espn`, `kalshi`, `manual`); ADR-119 line ~18263 promised `['weather_observation']` for NOAA but NOAA does not yet exist as a row. Without backfill, Item 7 ratifies an unenforced rule. V2.46 commits to a Cohort 5+ follow-on migration (issue **N1**) populating: `espn → ['game_state']`, `kalshi → ['market_snapshot']`. **The `manual` source row is excluded from N1 per session 91 user adjudication (B=(a)) resolving the Item 2 ∩ Item 7 boundary contradiction Glokta surfaced**: `manual` source-rows do NOT write directly to `canonical_observations`; the manual-override path uses `canonical_match_overrides` (slot 0074) per V2.44 Item 2 rebind discipline. `manual.authoritative_for` may remain NULL, or be populated only if/when a future workflow legitimately requires direct `canonical_observations` writes by the manual source — that decision belongs to the workflow design, not V2.46. Future external-source rows (e.g., `noaa_nws` for ADR-119 Phase 1) ship with their `authoritative_for` populated at insert time.
    3. **Override path (Galadriel 7a):** operator-override scenarios bypass the writer's reject-on-mismatch via a separate code path with audit trail (mirrors the slot 0073 `canonical_match_log` rebind discipline from V2.44 Item 2). Reject-as-default is canonicalized; **no `shadow_mode` toggle ships in V2.46** — keep enforcement loud. If a config-rollout case bites in production, amend then.
  - **Item 9 — Tagging pipeline as Phase 4-5 prerequisite (canonicalize 4 sub-rules).** Several `observation_kind` values require a tagging pipeline to map raw source data → `canonical_event_id` before any rows can be ingested into `canonical_observations`. The tagging pipeline itself is not designed in V2.46 — it is a Phase 4-5 multi-cohort effort with per-kind sophistication ramping. Sub-rules:
    - **9a — Vocabulary-reservation vs ingestion-gate (Q3 PM adjudication; resolves Holden A1).** `ck_canonical_observations_kind` retains the 6-kind enumeration shipped in slot 0078 (`game_state`, `weather`, `poll`, `econ`, `news`, `market_snapshot` — verified live via MCP) as a **vocabulary reservation**. The Pattern 73 SSOT "kind requires projection" rule (V2.45 Item 4) is enforced at the writer layer via a kind-registry assertion: `canonical_observations_writer` rejects `INSERT` for any kind whose per-kind projection table does not exist OR whose tagging pipeline is not registered (registry lookup, similar shape to `scheduler_status` pattern). Vocabulary-reserved-but-not-ingestible is a recognized state. The CHECK admits the vocabulary; the writer registry gates ingestion. Implementation tracked at issue **N2**. Holden A1 finding directly addressed: slot 0078's CHECK pre-loads 4 unprovisioned kinds (`weather`, `poll`, `econ`, `news` lack projection tables); this approach resolves the SSOT pre-violation without Migration 0085 narrow-then-re-expand churn (which would be Pattern-87-uncomfortable and operationally heavy for zero benefit since `canonical_observations` is currently 0 rows + writer feature-flagged off).
    - **9b — Tagging-quality threshold form.** Each kind goes live when its tagger achieves confidence-calibrated F1 ≥ X_kind on a labeled holdout set. V2.46 codifies the *form*; X_kind values belong in per-kind cohort design. Below-threshold tagger rows do not promote raw → `canonical_observations`.
    - **9c — Rollback runbook (manual order until N3 lands).** If a tagger floods `canonical_observations` with mistags, rollback discipline until FK polarity harmonization in Cohort 5+ (issue **N3**): (1) `DELETE FROM temporal_alignment` where the bad observations appear in `observation_a_id` or `observation_b_id` (current FK shape: `NO ACTION` default per live MCP — DELETE on `canonical_observations` blocked otherwise); (2) **Pre-N3:** `canonical_event_phase_log` rows are unaffected — the `evidence_observation_id` column does not yet exist on the live table (slot 0079 shipped a phase-transition log without an evidence FK; session 91 user adjudication A=(2) rescopes N3 to ADD this column preserving session-90 input-memo intent). **Post-N3:** `evidence_observation_id` exists with `ON DELETE SET NULL`; observation deletion auto-NULLs phase-log evidence references and the phase-transition record (`transition_at`, `previous_phase`, `new_phase`, `changed_by`, `note`) is preserved as audit history; no manual UPDATE needed; (3) `DELETE FROM canonical_observations` (composite-FK CASCADE on `canonical_observation_event_links` handles link rows automatically per V2.43 Item 3 invariant flow-through to slot 0084). N3 issue commits to ON DELETE CASCADE on `temporal_alignment` FKs + ADD `canonical_event_phase_log.evidence_observation_id` BIGINT NULL FK to `canonical_observations(id, ingested_at)` with ON DELETE SET NULL (column-add + polarity simultaneously) to make this ergonomic.
    - **9d — Cohort-sequencing principle.** Kinds activate in tagging-complexity order: `game_state` (trivial — implicit FK chain via `games.canonical_event_id` + slot 0080) → `market_snapshot` (trivial *given* upstream matcher slots have shipped — Galadriel 9b strengthening; the per-kind tagging pipeline cohort sequence has prerequisites that may belong to other cohorts; here the matcher must ship `canonical_market_links` rows before market_snapshot tagging is trivial) → `weather` (moderate — geo + temporal proximity to canonical_events with location data) → `poll` (moderate — entity linking via `canonical_entity`) → `econ` (moderate — econ_indicator → affected `canonical_markets` → `canonical_events`; many-to-many via `canonical_observation_event_links`) → `news` (hard — NER + entity linking + event linking + confidence scoring; many-to-many; news fans out across multiple canonical_events).
    - **9e — Re-tagging discipline (Galadriel 9a strengthening; Pattern-6-immutable-versioning by analogy).** When a tagging pipeline v1 ingests N observations with a given primary `canonical_event_id`, then v2 ships with better entity-linking and should re-tag some to a different `canonical_event_id`: re-tagging is **INSERT** into `canonical_observation_event_links` with a new `linked_by` value (the v2 tagger's ID), **NOT UPDATE**. The denormalized `canonical_primary_event_id` column on `canonical_observations` is updated by the reconciler when a new `link_kind = 'primary'` row supersedes the previous one. This is a Pattern-6-immutable-versioning-by-analogy operation — append-only audit trail, deterministic supersession. Symmetric with the rest of the canonical layer's append-only discipline (slot 0073 match-log, slot 0079 phase-log, etc.).
  - **Item 10 — Universal nouns / typed nouns / relationship verbs / audit ledgers framing (DESCRIBE-ONLY, not canonicalize; Q5 PM adjudication user-confirmed).** V2.46 USES the vocabulary in narrative prose (this amendment + the companion doc) but does **NOT** canonicalize as a binding architectural rule. Three Galadriel-identified ambiguities make the framing unsuitable for SSOT canonicalization: (i) `observation_kind` is *itself* a discriminator that effectively encodes a polymorphic relationship — the universal noun has a column that does verb-table work; (ii) `canonical_event_links` does double duty (canonical_events ↔ canonical_markets AND canonical_events ↔ platform events smashed into one table); (iii) `canonical_match_overrides` is operator-state-as-typed-noun, not a relationship — same structural ambiguity Item 2 is trying to keep OUT of `observation_kind`. Pattern 73 SSOT discipline argues against canonicalizing a vocabulary with three known leaks. Future amendment after Cohort 5+ pressure-tests the framing against real cross-domain expansions may revisit canonicalization.
  - **Cross-cutting findings (canonicalize as named patterns).**
    - **CC1 — The `canonical_observations` writer-discipline triad (Galadriel 4a).** Items 2 ∩ 3 ∩ 7 are three discriminators in **one gate**, not three independent rules. A row enters `canonical_observations` iff: (i) its source is `authoritative_for` the kind (Item 7), (ii) the kind is an external-world signal (Item 2), AND (iii) the row is resolution-relevant + tagged (Item 3). Naming the triad explicitly makes future writer code reviews and code-gen-style writers tractable. The N2 writer-registry assertion implementation enforces this triad as a unified gate.
    - **CC2 — The 2-prerequisite kind-activation gate (Galadriel 4b).** A new `observation_kind` needs **BOTH** a per-kind projection table (V2.45 Item 4 SSOT rule) **AND** a tagging pipeline registered (Item 9 / Q3 writer registry) before any ingestion. The gate makes the dependency explicit; without it, future cohort planners may add the projection table (cheap) + declare the kind "supported" + find that no observations actually flow because the tagging pipeline is incomplete.
    - **CC3 — V2.45 erratum on kind enum spelling (D2).** V2.45 narrative on the kind enumeration carried aspirational longer names; the shipped enumeration in slot 0078 + live `ck_canonical_observations_kind` (verified via MCP, session 91) uses the shipped names. V2.46 codification uses the live names throughout. **No DDL change required** — the drift is text-only and self-resolves by V2.46 ratifying live names. Per Pattern 87 spirit on amendment-text immutability, V2.45's text remains as-shipped; this erratum is the canonical bridge.
  - **Cohort 5+ deferred backlog from V2.46 (5 issues to file at PR creation).**
    - **N1** — `authoritative_for` backfill migration: populate the 3 existing `observation_source` rows + future source rows. Cohort 5+ scope alongside writer activation (canonical_observations writer feature flag flip from `false` → `true`).
    - **N2** — Writer-side kind-registry assertion implementation (Q3 enforcement). `canonical_observations_writer` rejects `INSERT` for any kind whose per-kind projection table does not exist OR whose tagging pipeline is not registered.
    - **N3** — `temporal_alignment` FK polarity harmonization (`ON DELETE CASCADE` on `observation_a_id`/`_b_id` FKs; currently default `NO ACTION` per live MCP) **+ ADD `canonical_event_phase_log.evidence_observation_id` BIGINT NULL FK** to `canonical_observations(id, ingested_at)` with `ON DELETE SET NULL` (rescoped per session 91 user adjudication A=(2): preserves session-90 input-memo intent that phase-log rows reference the observation triggering the transition; slot 0079 shipped a phase-transition-only log without this column — N3 adds it as the architectural-intent realization, not a polarity-flip on an existing FK). Makes tagging-pipeline-rollback (Item 9c) ergonomic + delivers the architectural intent. Composite FK shape required per V2.43 Item 3 invariant flow-through (FK target is partitioned with composite PK `(id, ingested_at)`).
    - **N4** — `market_trades.canonical_event_id` direct FK + matcher-link populate logic. Cohort 5+ alongside trade poller activation; mirrors slot 0080 pattern (`game_states.canonical_event_id` + `games.canonical_event_id` nullable BIGINT FK with Pattern 84 NOT VALID + VALIDATE if populated, plain on empty). Gives ML/strategy code canonical-event filtering on trades without `canonical_observations` volume cost. Path (d) per user adjudication on Q6.
    - **N5** — `v_*` lowercase view-prefix standardization for canonical-layer views (operational/SCD/backtest views OUT of scope per user direction). Renames target ~19 legacy un-prefixed views in the broader system; canonical-layer-relevant scope covers 4 production-code references in `crud_analytics.py`, `connection.py`, `tests/integration/database/test_edge_lifecycle_view.py`, `tests/e2e/schedulers/test_kalshi_poller_e2e.py`. V2.47 / Phase 5 scope, low priority.
  - **Open V2.46 forward-pointers (deferred from V2.46 scope):**
    - Per-kind raw-staging table shapes (deferred to per-kind cohort issues per Q2)
    - Per-kind T_kind threshold values (Item 3 form-vs-values split per Q1)
    - Per-kind X_kind tagging-quality thresholds (Item 9b form-vs-values split)
    - Phase 4-5 tagging pipeline implementation (per-kind cohort design)
    - Item 10 canonicalization revisit after Cohort 5+ stresses the framing against real cross-domain expansions
    - `market_trades` inclusion-as-`observation_kind` revisit if/when ML strategy actually wants cross-temporal-alignment of trades with game_states (currently path (d) keeps it as a Layer-2 sibling outside the canonical_observations vocabulary)
  - **Cross-references:** session 91 V2.46 design-review memos (`memory/design_review_v246_galadriel_memo.md`, `memory/design_review_v246_holden_memo.md`); PM synthesis (`memory/design_review_v246_synthesis.md`); session 90 binding input memo (`memory/design_review_v246_input_memo.md`); ADR-118 V2.45 Items 1/4/6/8 (the ratified architecture-shipping items V2.46 forward-pointed at); slot 0078 (`canonical_observations` partitioned parent + `ck_canonical_observations_kind` 6-kind enumeration — site of V2.45 D2 drift + Holden A1 finding); slot 0073 (`canonical_match_log` restricted-API for rebind audit — pattern for Item 7 override path); slot 0074 (`canonical_match_overrides` + `canonical_match_reviews` — operator-state tables Item 2 cites as exclusion case); slot 0075 (`observation_source.authoritative_for` JSONB column shape — Item 7 source); slot 0079 (`canonical_event_phase_log` audit ledger — Item 9c rollback runbook + Layer 4.5 sub-layer); slot 0080 (`game_states.canonical_event_id` + `games.canonical_event_id` — pattern N4 mirrors); slot 0084 (`canonical_observation_event_links` + redesigned `temporal_alignment` — Item 9e re-tagging discipline implementation surface); V2.43 Item 3 (composite-FK invariant flow-through — applies to N3 FK harmonization); V2.44 Item 2 (matcher rebind via `canonical_match_log` — pattern for Item 7 override path); ADR-119 Part 2 § Cohort 9 weather Phase 1 (deferred under Option D — tagging-pipeline cohort sequencing per Item 9d); DEVELOPMENT_PATTERNS Pattern 73 (SSOT — Item 7 + 9 + cross-cutting CC2) + Pattern 86 (Living-Doc Freshness Markers — required at top of new companion doc) + Pattern 87 (Append-only migrations — V2.46 ships ZERO edits to migrations 0001-0084, including the slot 0078 CHECK that pre-loads 4 unprovisioned kinds — vocabulary-reservation carve-out per Item 9a + Holden A1) + Pattern 6 (Immutable Versioning — by analogy for Item 9e re-tagging discipline) + Pattern 84 (NOT VALID + VALIDATE — referenced for N4 future migration shape).
  - **Slot-ordering constraint:** v2.46 ships ADR amendment text + `docs/database/CANONICAL_LAYER_RELATIONSHIPS.md` (NEW companion doc) in a single PR. **No new migrations** ship under v2.46. **No further Cohort 4 work** is queued (Cohort 4 closed informally on V2.45 / PR #1144 merge per V2.45 Item 1). **Cohort 5+** picks up the 5 deferred issues (N1-N5) plus the V2.45-deferred work (per-domain `temporal_alignment` views per #1141, source-pipeline writer wiring for `canonical_observations`, reconciler manual-CLI slot, matcher slot per V2.44 contract).
**Changes in v2.45:**
- **ADR-118 COHORT 4 INFORMAL CLOSE + CANONICAL LAYER REDESIGN (session 90):** Codifies four user-adjudicated decisions from session 90's architectural reasoning conversation, triggered by a slot 0083 build-spec premise check that revealed `temporal_alignment` was 0-rows (writer existed in code but never registered in scheduler_status; pollers had been producing 41,993 game_states + 135,023 market_snapshots without the alignment writer picking them up). The premise check escalated into a fundamental re-examination of the canonical layer architecture; user-adjudicated Path D = defer slot 0083 + redesign temporal_alignment + ratify a substantive V2.45 + design-review V2.46 next session. Implementation lands in **Migration 0084 / slot 0084** as a single PR carrying schema redesign. Comprehensive architectural reasoning capture lives in `memory/design_review_v246_input_memo.md` (binding input to V2.46). Items 2/3/5/7/9/10 of that memo are user-adjudicated in principle but deferred to V2.46 design-review (issue #1143) for canonicalization via Galadriel + Holden + Scheherazade + Glokta pipeline.
  - **Item 1 — Cohort 4 informal close + slot 0083 deferral.** Cohort 4 closes with the slots already shipped (0078 + 0079 + 0080 + 0082); slot 0083 work (`v_temporal_alignment` view rewire) is rescoped + deferred to Cohort 5+ alongside source-pipeline writer wiring per **issue #1141**. The original V2.43 Item 2 "5-slot Cohort 4" framing is hereby re-cohorted: the surviving 4 slots define Cohort 4's actual ship; the deferred slot becomes Cohort 5+ work. The slot 0083 hole in the alembic chain (`down_revision` jumps 0082 → 0084) is intentional and matches the slot 0081 hole shape from V2.43 Item 2 (slot 0081 retired into 0080); future readers should expect TWO intentional holes in the chain (0081, 0083) corresponding to two slot retirements/deferrals.
  - **Item 4 — Drop `canonical_observations.payload` column entirely.** Per-kind projection tables (`game_states`, `market_snapshots`, future `weather_observations` / `poll_releases` / `econ_prints` / `news_events`) carry typed columns + JSONB residue where needed (e.g., `game_states.situation` + `game_states.linescores` for ESPN's untyped fields; `market_snapshots` has none because Kalshi is fully typed). The `canonical_observations.payload` JSONB column was redundant with what per-kind tables already carry; dropping it makes `canonical_observations` a pure lineage / linkage table. **NEW Pattern 73 SSOT discipline rule:** adding a new `observation_kind` value to `OBSERVATION_KIND_VALUES` requires its projection table to exist BEFORE the vocabulary expansion. This rule guards against the "vocab admits a kind we have no projection for" failure mode that the original `payload` column papered over.
    - **Operational impact at slot 0084 land time:** `canonical_observations` has 0 rows → `DROP COLUMN payload` is zero-data-loss. Pattern 87 compliant (slot 0078 file unchanged; slot 0084 is a separate forward migration). Storage win: payload was the heaviest column; dropping it makes `canonical_observations` small + fast + tightly-indexable.
    - **Back-reference (Cohort 5+):** per-kind tables get `observation_id BIGINT REFERENCES canonical_observations(id)` (deferred to Cohort 5+ when source pipelines wire in; not in slot 0084 scope). The FK points typed-table → universal-record, not the reverse — avoids polymorphic-FK problem.
  - **Item 6 — `temporal_alignment` redesigned as PURE LINKAGE TABLE.** The existing 20-column denormalized fact-table shape (slot 0035-era design predating canonical_observations; sport-specific typed columns `home_score` / `away_score` / `period` / `clock` / `yes_ask_price` / `no_ask_price` etc.) is replaced with a ~9-column shape: two FKs into `canonical_observations` (`observation_a_id`, `observation_b_id`), a denormalized `canonical_event_id` for hot-path lookups (mirroring slot 0082's polarity), `time_delta_seconds`, `alignment_quality`, `aligned_at`, `created_at`, plus PK. Per-domain typed views (`v_temporal_alignment_sports` / `v_temporal_alignment_weather` / etc.) deferred to Cohort 5+ alongside writer wiring per **issue #1141**.
    - **Why current (pre-V2.45) design is wrong-shape:** `temporal_alignment` was designed in Cohort C0 (issue #722, session 49) when canonical_observations didn't exist. The writer's INSERT denormalizes everything from market_snapshots + game_states into a single fat row (~18 columns of source data + 2 of metadata). That made sense pre-canonical-layer; it's redundant + sport-specific now.
    - **Why current design blocks multi-domain:** sport-specific columns (home_score, period, etc.) preclude future weather × market alignment, election poll × market alignment, etc., without parallel sport-specific tables forever or a wide-row Frankenstein. The pure-linkage redesign cleanly admits multi-domain pairings without schema growth.
    - **New shape constraints:** `uq_alignment_pair UNIQUE (observation_a_id, observation_b_id)` for matcher idempotence; `ck_distinct_observations CHECK (observation_a_id <> observation_b_id)` because self-pair is meaningless; `ck_alignment_quality CHECK (alignment_quality IN ('exact','good','fair','poor','stale'))` as Pattern 73 SSOT canonical home for the 5-value vocabulary until Cohort 5+ CRUD module ships.
    - **FK polarity:** `observation_a_id` / `observation_b_id` use PG default (`NO ACTION`) on delete — operator must explicitly drop alignments before dropping observations (matcher owns the lifecycle). `canonical_event_id` uses `ON DELETE SET NULL` mirroring slot 0078 / slot 0082 polarity (alignment history outlives canonical_event delete).
    - **Operational impact:** `temporal_alignment` has 0 rows → `DROP TABLE` + `CREATE TABLE` with new shape is zero-data-loss. Cohort 5+ writer rewrite picks up the new shape.
  - **Item 8 — `canonical_event_id` rename + multi-event link table.**
    1. **Rename `canonical_observations.canonical_event_id` → `canonical_primary_event_id`** to make denormalized-as-primary-tag semantics explicit. The renamed column on `canonical_observations` becomes a hot-path read; the link table (below) carries full multi-event semantics.
    2. **Add `canonical_observation_event_links` table** for many-to-many multi-event tagging (news fans out across multiple canonical_events; econ prints affect dozens of econ markets). Columns: `observation_id BIGINT NOT NULL REFERENCES canonical_observations(id) ON DELETE CASCADE`, `canonical_event_id BIGINT NOT NULL REFERENCES canonical_events(id) ON DELETE RESTRICT`, `link_kind VARCHAR(16) NOT NULL CHECK (link_kind IN ('primary','secondary','derived','speculative'))`, `confidence NUMERIC(5,4)` (NULL for manual tags; CHECK 0-1 range), `linked_at TIMESTAMPTZ NOT NULL DEFAULT now()`, `linked_by VARCHAR(64) NOT NULL`. Composite PK `(observation_id, canonical_event_id)` for idempotence. Partial unique index `idx_link_one_primary_per_observation ON (observation_id) WHERE link_kind = 'primary'` enforces "at most one primary link per observation," mirroring the `canonical_observations.canonical_primary_event_id` denormalized column's "single primary tag" invariant.
    - **Relationship to `canonical_primary_event_id`:** the column on canonical_observations is the denormalized "primary tag" (mirrors the `link_kind = 'primary'` row in the link table). Reconciler (Cohort 5+) asserts agreement (similar shape to V2.44 Item 5's dim+fact reconciliation pattern). The column gets a hot-path read; the link table gets full multi-event semantic.
    - **Pattern 73 SSOT:** `link_kind` vocabulary lives ONLY in this slot's CHECK constraint as the canonical home until Cohort 5+ ships the corresponding CRUD module + constants tuple `CANONICAL_OBSERVATION_LINK_KINDS`. Slot 0084 is a pure DDL slot; no Python imports of the vocabulary land this slot.
  - **V2.43 Item 3 composite-FK invariant flow-through (applies to Items 6 + 8 above; codified post-build via S55 MCP-grounded discovery).** The single-column UNIQUE / CHECK / PK shapes shown in Items 6 + 8's narrative text (e.g., `uq_alignment_pair UNIQUE (observation_a_id, observation_b_id)`; `ck_distinct_observations CHECK (observation_a_id <> observation_b_id)`; link table composite PK `(observation_id, canonical_event_id)`) are **conceptual shapes**; the actual Migration 0084 ships *expanded composite* shapes because `canonical_observations` has composite PK `(id, ingested_at)` per V2.43 Item 3's partition-key invariant. PG rejects single-column FK references to composite PK targets at upgrade time (`InvalidForeignKey: there is no unique constraint matching given keys for referenced table`). The invariant flow-through expands ALL FK / UNIQUE / PK references touching `canonical_observations` to include `(observation_id, observation_ingested_at)`:
    - `temporal_alignment` ships TWO composite FKs: `FOREIGN KEY (observation_a_id, observation_a_ingested_at) REFERENCES canonical_observations(id, ingested_at)` + the same for `observation_b_*`. The `uq_alignment_pair` UNIQUE expands to `(observation_a_id, observation_a_ingested_at, observation_b_id, observation_b_ingested_at)` (4 columns, not 2). The `ck_distinct_observations` CHECK expands to `NOT (observation_a_id = observation_b_id AND observation_a_ingested_at = observation_b_ingested_at)`.
    - `canonical_observation_event_links` ships composite FK `FOREIGN KEY (observation_id, observation_ingested_at) REFERENCES canonical_observations(id, ingested_at)`. The PK expands to `(observation_id, observation_ingested_at, canonical_event_id)` (3 columns, not 2). Partial unique `idx_link_one_primary_per_observation` similarly keys on `(observation_id, observation_ingested_at) WHERE link_kind = 'primary'` to ensure cross-partition primary uniqueness.
    - PG also auto-creates child-partition FK replicas (one per monthly partition of `canonical_observations`) — expected partition-routing behavior; verified at build time via `pg_constraint`.
    - **Why this matters as forward-pointing documentation:** future readers of V2.45 Items 6 + 8 see the conceptual shape; future readers of Migration 0084 see the implementation shape; this paragraph closes the gap. Per Pattern 87 spirit on amendment-text immutability, V2.45's Item 6 + 8 narrative is preserved as written; the flow-through paragraph is the canonical bridge between conceptual + implementation shapes.
  - **V2.46 forward pointer (issue #1143):** session 91+ design-review will canonicalize Items 2/3/5/7/9/10 architectural framing via Galadriel + Holden + Scheherazade + Glokta pipeline. Binding input at `memory/design_review_v246_input_memo.md`. Items deferred to V2.46:
    - Item 2 — `observation_kind` boundary (external-world signals only; internal state goes elsewhere)
    - Item 3 — `canonical_observations` is filtered, not universal sink (resolution-relevant + tagged subset only)
    - Item 5 — View-naming convention (`v_<operation>` vs `v_<thing1>_<verb>_<thing2>` vs `v_<thing>_enriched` forms)
    - Item 7 — `observation_source.authoritative_for` reaffirmation (slot 0075 inheritance)
    - Item 9 — Tagging pipeline as Phase 4-5 prerequisite (per-kind sophistication ramping)
    - Item 10 — Universal nouns / typed nouns / relationship verbs framing (canonicalize or describe?)
    - Companion deliverable: `docs/database/CANONICAL_LAYER_RELATIONSHIPS.md` (4-layer relationship graph + traversal examples + verb-table catalog)
  - **Cross-references:** `memory/design_review_v246_input_memo.md` (binding architectural reasoning); session 49 issue #722 (original temporal_alignment shape — superseded by V2.45 Item 6); slot 0078 / `memory/build_spec_0078_pm_memo.md` (canonical_observations parent — payload column dropped; canonical_event_id renamed); slot 0080 / 0082 (game_states / games / temporal_alignment canonical_event_id columns — INDEPENDENT of canonical_observations rename, verified at build time via MCP `information_schema.columns`); ADR-118 V2.43 Items 1-6 (Cohort 4 scope lock; V2.45 Item 1 supersedes Item 2's "5-slot Cohort 4" framing); ADR-118 V2.42 sub-amendment B (canonical_events FK polarity convention — preserved in V2.45 link table FK shapes); ADR-118 V2.44 Items 1-6 (matcher-atomicity contract — preserved; slot 0083 implementation deferred per V2.45 Item 1); DEVELOPMENT_PATTERNS Pattern 73 (SSOT — new discipline rule for `observation_kind` projection-table prerequisite) + Pattern 87 (Append-only migrations — slot 0084 makes ZERO edits to migrations 0001-0082) + Pattern 86 (Living-Doc Freshness Markers — DATABASE_SCHEMA_SUMMARY V2.4 refresh ships in same PR).
  - **Slot-ordering constraint:** v2.45 ships ADR amendment text + Migration 0084 + DATABASE_SCHEMA_SUMMARY V2.4 in a single PR. **No further Cohort 4 work** is queued; Cohort 4 informally closes on this PR's merge. **Cohort 5+** picks up: per-domain `temporal_alignment` views (issue #1141), source-pipeline writer wiring for `canonical_observations` (currently feature-flagged off), reconciler manual-CLI slot, matcher slot per V2.44 contract, V2.46 design-review work for the deferred Items 2/3/5/7/9/10 (issue #1143).
**Changes in v2.44:**
- **ADR-118 COHORT 5+ MATCHER-ATOMICITY CONTRACT (Issue #1135, session 89):** Codifies the session 89 #1135 council synthesis (Galadriel + Holden 2-agent council + PM synthesis) on `canonical_event_id` binding consistency across `game_states.canonical_event_id` (slot 0080) + `temporal_alignment.canonical_event_id` (slot 0082). Amendment is narrative + acceptance-criteria codification only; no new tables ship under v2.44. Implementation lands in **slot 0083** (Cohort 4 close) for the `temporal_alignment` INSERT trigger + dual-column view, AND in the **Cohort 5+ matcher slot** for the `games` write path + `game_states` INSERT trigger. Closes Issue #1135.
  - **Item 1 — Boundary: ratification of V2.43 sub-decision #4 (Items 1-3, 5) + one new design point (Item 4).** V2.44 ratifies V2.43 sub-decision #4's "Dim + fact canonical linkage is BOTH" commitment + the detailed shape in V2.43's § "Dim + fact canonical linkage: both, with reconciliation" subsection — that's Items 1-3 and Item 5 below. **Item 4 (dual-column view shape) is genuinely new architecture** that emerged from the session 89 council's SCD-2 conflation analysis (Holden's frame); the V2.43 § "Stable consumer contract (the key architectural commitment)" subsection committed to a single stable interface, and Item 4's dual-column shape codifies the narrower contract Holden's analysis warranted. Honest catalog: Items 1-3 + 5 = ratification; Item 4 = new architecture. The #1135 council surfaced as principled divergence (Galadriel: Option A denormalized cache via trigger + reconciler health-check; Holden: Option C reconciliation + dim/fact split + dual-column view). PM synthesis (`memory/design_review_1135_synthesis.md`) collapsed the divergence by referencing the existing v2.43 commitment that both council members had under-quoted, AND adopting Holden's dual-column shape as the SCD-2-conflation-prevention discipline.
  - **Item 2 — Matcher write surface = `games.canonical_event_id` only.** Single-row UPDATEs gated by `WHERE canonical_event_id IS NULL` for steady-state binds. Rebind operations (overwriting an existing non-NULL value) go through a separate code path that emits a `canonical_match_log` entry per slot 0073's restricted-API discipline. Holden's data-integrity argument carries: cross-table atomic transactions (Option B in council framing) pay lock-cost without the eventual-consistency guarantee they appear to deliver, because new fact rows always land between matcher cycles regardless of transactional grouping. Narrower write surface than Galadriel's initial Option A framing proposed; the matcher touches one column on one row.
  - **Item 3 — Writer-side trigger discipline (split across two slots):**
    - `game_states` INSERT trigger populates `game_states.canonical_event_id` from `games.canonical_event_id` (joined via `game_id`) — ships in the **Cohort 5+ matcher slot** (when the matcher first writes to `games.canonical_event_id`, the `game_states` writer trigger needs to be present so subsequent `game_states` INSERTs populate the fact column from the dim source of truth)
    - `temporal_alignment` INSERT trigger populates `temporal_alignment.canonical_event_id` from `game_states.canonical_event_id` (joined via `game_state_id`) — ships in **slot 0083** (alongside the view rewire so the dual-column contract is self-enforcing from day 1)
    - Trigger pattern matches the existing `set_updated_at()` precedent from slot 0076 (V2.42 sub-amendment A retrofit; PR #1098): BEFORE INSERT, FOR EACH ROW, plpgsql, respects caller-supplied non-NULL values (allows the future Cohort 5+ matcher to bypass the trigger by passing the value directly during a coordinated bind). Note: the audit-column convention itself was flagged in V2.42 sub-amendment A as a future DEVELOPMENT_PATTERNS promotion candidate but has not been promoted; Pattern 87 V1.40 is "Append-Only Migration Files," not the audit-column convention.
    - Rationale for split timing: each trigger ships when the FIRST write to the corresponding fact column under the new contract would occur. Shipping the `game_states` trigger early (in slot 0083 or slot 0084) would create a trigger that fires on every `game_states` poll (high frequency) but does nothing useful until matcher writes start landing — wasted overhead.
  - **Item 4 — `v_temporal_alignment` view shape: dual-column.** The view exposes BOTH:
    - `canonical_event_id_at_alignment` — denormalized fact, **point-in-time semantic** (populated by the `temporal_alignment` INSERT trigger from `game_states.canonical_event_id` at INSERT time); use for ML feature extraction, point-in-time replay, causal-correctness scenarios where "what did the system know at time T?" is the relevant question
    - `canonical_event_id_current` — dim, **current-state semantic** (JOIN to `games.canonical_event_id` via `temporal_alignment.game_id`); use for operational queries where "is this snapshot tied to the canonical event in effect right now?" is the relevant question
    - View docstring states which audience each column serves; consumers self-select. Direct reads of `temporal_alignment.canonical_event_id` (without going through the view) return the point-in-time value AND skip the JOIN cost — acceptable for hot-path ML feature pipelines that have already self-selected the point-in-time semantic.
    - **Why dual-column is load-bearing:** SCD-2 versioning of `game_states` makes single-derived-column views semantically ambiguous because `temporal_alignment.game_state_id` references a SPECIFIC SCD-2 version of `game_states`. A single derived column would silently conflate point-in-time vs current-state semantics; the dual-column shape makes the distinction explicit at the consumer contract surface. Holden surfaced this finding; Galadriel's single-derived-column shape in the original Option A framing would have shipped the conflation.
    - Slot 0083 build spec (`memory/build_spec_0083_pm_memo.md`) is binding-and-ready and specifies the view body verbatim.
  - **Item 5 — Reconciler is the sole enforcement surface.** **NO** CHECK constraint or cross-table trigger asserting column agreement:
    - PostgreSQL CHECK constraints cannot reference other tables (PG 12+ allows subqueries only via the `CREATE ASSERTION` syntax, which is itself unimplemented across PG versions Precog targets)
    - Cross-table triggers add write-path latency on every INSERT to `temporal_alignment` (one of the highest-volume tables in the system) and defeat the single-row-write atomicity that Item 2's matcher discipline preserves
    - The `reconcile_canonical_linkage` reconciler (per V2.43 § "Dim + fact canonical linkage: both, with reconciliation" / Reconciliation contract paragraph) gains a query shape comparing `(games.canonical_event_id, game_states.canonical_event_id_for_current_row, temporal_alignment.canonical_event_id_per_game_state)` and surfaces divergences as **warning-severity** entries in `canonical_reconciliation_results` per V2.43 Item 5 baseline (warning-only on Cohort 4+ ship; tightening gated to Cohort 6+ post-baseline)
    - One reconciler, multiple query shapes (matcher-atomicity check joins canonical-observations-vs-projections checks under shared severity-tiering logic). Operationally simpler — one cron, one alert pipeline, multiple SELECT shapes. Ships in Cohort 5+ alongside the matcher.
  - **Item 6 — V2.43 Item-numbering disambiguation (recorded for future readers):** The original Issue #1135 body cited "V2.43 Item 6" for the composite-FK invariant. Live ADR text shows V2.43 Item 6 is the Phase B.5 sports/weather split; the composite-FK invariant is V2.43 sub-decision #3 (§ "Decision summary" item #3); the binding-architecture commitment ratified by V2.44 is V2.43 sub-decision #4 (§ "Decision summary" item #4) + V2.43 § "Dim + fact canonical linkage: both, with reconciliation" subsection. This is **correction-by-amendment**, not editing of V2.43 text (which remains as-shipped per Pattern 87 spirit on amendment text immutability, and Pattern 73 SSOT — V2.44 is the canonical home for the corrected reference; future readers cite V2.44 Item 6 + sub-decision #4, not the issue body's stale citation). **Citation discipline note (Pattern 86):** prior drafts of this V2.44 amendment cited specific line numbers (17326, 17849-17859, 17857) for V2.43 anchors; per Pattern 86 (Living-Doc Freshness Markers) line numbers decay at every prepend. This V2.44 ships with section-heading citations instead — stable across future ADR amendments.
  - **Cross-references:** session 89 council memos (`memory/design_review_1135_galadriel_memo.md`, `memory/design_review_1135_holden_memo.md`); PM synthesis (`memory/design_review_1135_synthesis.md`); slot 0083 build spec (`memory/build_spec_0083_pm_memo.md`); V2.43 sub-decision #4 (§ "Decision summary" item #4) + V2.43 § "Dim + fact canonical linkage: both, with reconciliation" subsection (the pre-existing commitment ratified here); slot 0073 (`canonical_match_log` restricted-API for rebind audit); V2.42 sub-amendment B (`canonical_events.game_id`/`.series_id` ON DELETE SET NULL polarity); V2.43 Item 4 (`canonical_reconciliation_results` new dedicated table); V2.43 Item 5 (reconciler warning-only severity baseline); slot 0076 (`set_updated_at()` generic trigger discipline precedent); Migration 0035 (`temporal_alignment.game_id` denormalized cache project precedent); DEVELOPMENT_PATTERNS Pattern 73 (SSOT) + Pattern 87 (append-only migrations) + Pattern 84 V1.42 (NOT VALID + VALIDATE for FK on populated tables) + Pattern 86 (Living-Doc Freshness Markers — applied to citation discipline; see Item 6).
  - **Slot-ordering constraint:** v2.44 ships ADR amendment text only. **Slot 0083 implementation** (`temporal_alignment` INSERT trigger + dual-column view + EXPLAIN ANALYZE artifact) is the binding-and-ready Builder work; build spec at `memory/build_spec_0083_pm_memo.md` is queued for session 90+ Builder dispatch and CLOSES Cohort 4 on merge. **Cohort 5+ matcher slot** (`games.canonical_event_id` write path + `game_states` INSERT trigger + `reconcile_canonical_linkage` matcher-atomicity query shape) is the corresponding Cohort 5 deliverable, dependent on canonical_observations soak baseline + matcher infrastructure (Cohort 3 slots 0071-0075 already shipped).
**Changes in v2.43:**
- **ADR-118 COHORT 4 SCOPE LOCK + COHORT NUMBERING CANONICALIZATION (session 86, codifying session 85 council adjudication):** Encodes 5 user-adjudicated decisions from the session 85 Cohort 4 4-agent design council (Galadriel + Holden + Miles + Uhura paired-cluster). Amendment is narrative + scope-lock + Phase B.5 criterion edit; no new tables ship under v2.43 itself — Cohort 4 build sequence begins at slot 0078 in session 86+.
  - **Item 1 — Cohort numbering canonicalization formally ratified.** The "Cohort N" labeling convention established informally in v2.39 (Cohort 4 = "event-state layer + canonical_observations") is hereby formal. Cohorts identify a SCOPE GROUPING; slot numbers float per-cohort and are assigned at build time. Future ADR text MUST refer to a cohort by its label + per-cohort slot range; do NOT assume any cohort owns a contiguous slot range across versions. Closes the v2.39 informality.
  - **Item 2 — Cohort 4 = 5-slot scope locked.** The session 85 council synthesis estimated 6 slots; session 86 user adjudication on Q5 collapses two of the nullable-FK retrofit slots into one. Final mapping:
    - **0078** — `canonical_observations` partitioned parent + 4-6 monthly partitions (RANGE on `ingested_at`) + composite PK `(id, ingested_at)` + 5 indexes + dedup UNIQUE on `(source_id, payload_hash)` + 3 SSOT vocabulary tuples (`OBSERVATION_KIND_VALUES` + `PARTITION_STRATEGY_VALUES` + `RECONCILIATION_OUTCOME_VALUES`) + `crud_canonical_observations.append_observation_row()` writer + ServiceSupervisor registration + `docs/operations/canonical_observations_runbook.md` skeleton
    - **0079** — `canonical_event_phase_log` table + FKs + CRUD module
    - **0080** — `game_states.canonical_event_id` + composite FK `(observation_id, observation_ingested_at)` + 2 partial indexes AND `games.canonical_event_id` + partial index, **combined into one migration** per session 86 user adjudication. Both are nullable-column-ADD on populated tables; PG11+ ADD COLUMN without DEFAULT is metadata-only (no row rewrite); single-migration shape acceptable for matched lock-discipline.
    - **0082** — `temporal_alignment.canonical_event_id` + partial index (currently 0 rows; single-phase fine)
    - **0083** — `v_temporal_alignment` view rewire (Option B plain view) + EXPLAIN ANALYZE artifact in PR + benchmark fixture; existing writer continues during soak; Option A (writer) fallback pre-authorized at p99>500ms threshold without council re-engagement
    - **Slot 0081 retired** (collapsed into 0080 per Q5 adjudication; remaining slots 0082/0083 retain their numbers — the 0081 hole is documented and intentional, NOT renumbered, per Pattern 73 SSOT discipline + Pattern 87 append-only spirit applied to slot numbering).
  - **Item 3 — Composite FK shape on partitioned `canonical_observations` PK is canonical.** Per the partitioned-PK constraint (RANGE on `ingested_at`, composite PK), every projection FK to `canonical_observations` MUST be composite `(observation_id, observation_ingested_at)`, not simple `observation_id` against a global UNIQUE. Trades API ergonomics for the partition-locking benefit (a global UNIQUE across partitions defeats the main partitioning value — locking + maintenance cost identical to unpartitioned). Document as Cohort 4 design pattern in slot 0078 docstring; promotion to DEVELOPMENT_PATTERNS post-build candidate.
  - **Item 4 — `canonical_reconciliation_results` ships as a NEW dedicated table (Option A), NOT as a discriminator on `evaluation_runs`.** Schema-grounded reasoning: `evaluation_runs.run_type` discriminator vocabulary (`'model'`, `'strategy'`, `'ensemble'` per Migration 0031 CHECK) is homogeneously ML/eval; reconciler runs have no `model_id` or `strategy_id` to populate (they compare canonical observations to per-kind projections — neither model nor strategy is involved), so reusing the table forces permanently-NULL FKs (a code smell). Project precedent (`backtesting_runs` shipping separately from `evaluation_runs` despite both tracking "a workflow with status + start/end + JSONB config" — see Migration 0031 Step 2) is the canonical pattern: dedicated table for invariant typed outputs. Reconciler outputs ARE invariant (anomaly count, severity, source/projection hashes, reconciled_at, inputs_compared count). Slot for `canonical_reconciliation_results` is **post-Cohort 4** (Cohort 5+ alongside automation-cadence reconciler; manual-CLI reconciler in Cohort 4 ships without persistent results table; Cohort 4 reconciler outputs ride on `system_health.details` JSONB + `alerts` until Cohort 5+ table lands).
  - **Item 5 — Reconciler severity baselines = warning-only on Cohort 4 ship.** The Cohort 4 manual-CLI reconciler ships with `severity=warning` alerts from day 1; first soak week establishes anomaly baseline; tightening to higher severity is gated to Cohort 6+ post-baseline. Rationale (Uhura frame): silent reconciler is worse than noisy reconciler during the highest-failure-likelihood post-deploy window. Counter-argument (zero-alert-until-baselined) considered and rejected — operator fatigue is recoverable; missed corruption is not.
  - **Item 6 — Phase B.5 exit criterion #4 split into #4a sports / #4b weather.** Phase B.5 exit criterion #4 (canonical text in `memory/plan_schema_hardening_arc.md`) was originally framed as a single sports+weather canonical-layer validation criterion (per v2.37 ADR-119 framing where weather Phase 1 was a Cohort 4-adjacent deliverable). v2.43 splits:
    - **#4a — sports canonical-layer validation:** gates Cohort 4 closeout (slots 0078-0083)
    - **#4b — weather canonical-layer validation:** defers to Cohort 9 (Phase 1 weather, ADR-119 Part 2, deferred under Option D — see `memory/project_weather_phase1_deferral.md`; midpoint reconsideration gate at ~session 90)
    Removes the phantom dependency between Phase B.5 closeout and Cohort 9 weather work that no longer happens in Cohort 4 under Option D. Without the split, Phase B.5 cannot exit until Cohort 9 ships, which would block every downstream phase indefinitely.
  - **Cross-references:** session 85 Cohort 4 design council memos (`memory/design_review_cohort4_galadriel_memo.md`, `memory/design_review_cohort4_holden_memo.md`, `memory/design_review_cohort4_miles_memo.md`, `memory/design_review_cohort4_uhura_memo.md`); PM synthesis (`memory/design_review_cohort4_synthesis.md`); session 86 user-adjudication 5-Q final picks; `memory/inventory_cohort_4_priorart.md`; `memory/project_weather_phase1_deferral.md`; ADR-119 Part 2 (Phase 1 weather sister ADR — deferred); Migration 0031 Step 2 (`backtesting_runs` precedent for Item 4 schema-shape decision); `evaluation_runs` live schema (queried via postgres-dev MCP at session 86 to validate Item 4 trade-off).
  - **Slot-ordering constraint:** v2.43 ships ADR amendment text only. Cohort 3 (slots 0071-0075) and the v2.42 retrofit slots 0076 (trigger retrofit, PR #1098) + 0077 (FK SET NULL, PR #1108) are ALL CLOSED on main as of session 85. Slot 0078 build begins immediately post-v2.43 merge.
**Changes in v2.42:**
- **ADR-118 AUDIT-COLUMN CONVENTION RATIFICATION (Issue #1007, session 79):** Sub-amendment A. All canonical-tier tables carry `created_at TIMESTAMPTZ NOT NULL DEFAULT now()` (already shipped Cohorts 1-2 + slot 0071; ratified for forward consistency). Main tables carrying `updated_at` MUST install a BEFORE UPDATE trigger using a generic `set_updated_at()` function — a nullable `updated_at` without a trigger is a misleading tombstone, either actively maintained or removed. Pattern 87 candidate flagged for DEVELOPMENT_PATTERNS promotion ("Audit-column convention for canonical-tier tables — created_at always; updated_at only with trigger"). Trigger retrofit migration ships at **slot 0076+** (post-Cohort 3 close); see #1074.
...
- **ADR-118 COHORT 2 AMENDMENT (session 73):** Encoded 5 user-adjudicated design decisions from the Holden + Galadriel 2-agent compressed council-variant validation (session 73) for the Cohort 2 `canonical_markets` table (Migration 0069). Amendment is narrative + DDL-tightening only; no new tables.
  ...
  - **Pattern 82 explicit non-application to `canonical_markets`** documented in prose ("`canonical_markets` has NO Pattern 82 application in Phase 1; per-platform market identity is owned by `canonical_market_links` (Migration 0072 — adjusted from 0071 in v2.41 5-slot adjudication), not by typed back-ref columns"). Ratifies `canonical_event_id BIGINT NOT NULL REFERENCES canonical_events(id) ON DELETE RESTRICT` with explicit rationale (NOT NULL precludes SET NULL; CASCADE would silently delete settlement-bearing markets; asymmetry with `canonical_event_participants.canonical_event_id` CASCADE intentional). Closes Holden Finding 6 + Galadriel Finding 4.
...
- `docs/foundation/MASTER_REQUIREMENTS_V2.27.md` (REQ-MODEL-EVAL-001, REQ-MODEL-EVAL-002)
...
- `docs/foundation/MASTER_REQUIREMENTS_V2.27.md` (REQ-ANALYTICS-003, REQ-REPORTING-001)
...
- `docs/foundation/MASTER_REQUIREMENTS_V2.27.md` (REQ-ANALYTICS-004, REQ-VALIDATION-003)
...
- `docs/foundation/MASTER_REQUIREMENTS_V2.27.md` (analytics requirements)
...
| **2 — Internal business key** | Composed inside this system from trusted internal facts | **Case-by-case — may or may not be UNIQUE; decide per column with explicit rationale** | `(sport, game_date, home_team_id, away_team_id)` composite matchup, `positions.position_key` (`POS-{id}` — SCD-2 version-stable surrogate per Pattern 80), `sports.sport_key` / `leagues.league_key` (human-readable natural keys) |
| **3 — External key** | Sourced from a platform/API we do not control | **ASSUMED NOT UNIQUE.** A Tier-3 UNIQUE is permitted ONLY against an immutable vendor contract guaranteeing uniqueness across the table's scope — documented in the migration | `games.espn_event_id`, `markets.kalshi_ticker`, `events.polymarket_condition_id`, `historical_games.cbbd_game_id`, `series.series_key` (*stores Kalshi `series_ticker`; updated Cohort 9 per ADR-119 Part 1 — scheduled rename to `external_series_ticker` in migration 0094*) |
...
- **Grep-able policy surface.** Columns suffixed `_id` where the prefix is a vendor name (`espn_`, `kalshi_`, `polymarket_`, `cbbd_`) are Tier 3; columns suffixed `_key` are *usually* Tier 2 (per ADR-116 Rule 2); columns named `id` are Tier 1. The ADR-116 naming convention becomes the visible manifestation of the three-tier taxonomy. **Exception flagged Cohort 9 (ADR-119 Part 1):** `series.series_key` looks Tier-2 by suffix but actually stores Kalshi's `series_ticker` — i.e., externally-sourced — and is therefore Tier 3 despite its name. Scheduled for rename to `external_series_ticker` in migration 0094 so the suffix convention becomes reliable again. Any future `_key` column MUST be audited at migration time to verify the value actually originates internally; a `_key` that stores a vendor value is a Tier-3 misclassification waiting to happen.
...
**Date:** April 22, 2026
**Status:** ✅ Accepted
**Phase:** Epic #972 (sibling to Epic #935) — Canonical Layer Foundation (multi-session arc, Phase 1 ships in sessions 71-74)
...
| **Level A entity** (see ADR-120, future) | Platform-scoped entity — `markets`, `events` today (planned rename to `platform_markets` / `platform_events` per ADR-120). Its Tier 1 PK is unique only within its platform. | `markets.id=1337` |
| **Level B entity** (see ADR-120, future) | Canonical entity — `canonical_markets`, `canonical_events`. Its Tier 1 PK is the project's durable identity across platforms. No Tier 3 (no external source). | `canonical_markets.id=7` |
...
New canonical kinds, domains, event_types, or roles land as INSERT rows in a migration (typically a seed-only migration), not as DDL changes. This is Pattern 81 — "Open canonical enum → lookup table over CHECK constraint" — canonical in DEVELOPMENT_PATTERNS V1.36 (promoted alongside Cohort 1 shipping).
...
4. **Pattern 82 V2 Forward-Only Direction Policy + mandatory load-bearing regression test (#1011 item 4).**
   ...
   > **Pattern 82 V2 wording (canonical):** see `DEVELOPMENT_PATTERNS.md § Pattern 82 V2 Forward-Only Direction Policy` for the rule, the rationale (preserves polymorphic-overloading future-extensibility per ADR-118 V2.38 decision #5), and the mandatory compensating mechanism (every Pattern 82 application MUST be paired with a load-bearing regression test). ADR-118 V2.40 ratifies this rule for the polymorphic-typed-back-ref family in this codebase; the canonical wording's home is `DEVELOPMENT_PATTERNS.md` per Pattern 73 SSOT (rules → `DEVELOPMENT_PATTERNS.md`, decisions → ADRs). Closes Pattern 73 violation flagged by claude-review on PR #1034 (#1036).
   >
   > **The canonical_entity instance's load-bearing test:** `tests/database/test_canonical_entity_polymorphic_invariants.py` (folded into #1021 scope). MUST NOT be skipped, retired, or admitted to any audit bypass set.
   >
   > **SCHEDULED RETIREMENT in V2.47 (cleanup epic Slot 5 / session 99 / #1155 closure):** the load-bearing test pin retires when canonical_entities.ref_team_id + trg_canonical_entity_team_backref are removed by Migration 0086 (Slot 2 / session 96). Path A per session 96 PM adjudication: test deleted at Slot 2; formal V2.40-pin retirement codified at V2.47. Pattern 82 V2 SCOPE NARROWING in V2.47 applies the rule to canonical_markets only post-cleanup-epic Slot 2.
...
- 4-slot framing (Galadriel, Elrond) preserved the v2.40 implementation table (see v2.40 implementation roadmap below) and honored the published-contract surface in `crud_canonical_markets.py:386` ("Migration 0071 = canonical_market_links").
...
The v2.40 implementation table (see v2.40 implementation roadmap below) is retained as the historical record of the v2.40 plan; this v2.41 amendment is the canonical post-adjudication mapping (Pattern 73 SSOT — the v2.40 table stays as the historical anchor, this amendment paragraph is the live mapping). Downstream slot numbers (0076+) shift by one slot accordingly: `canonical_events.lifecycle_phase` was already shipped in 0067 (see Issue #1015 stale-roadmap-entry tracker), so the 0076 line was already stale at v2.40 time and that staleness is unaffected by the v2.41 amendment.
...
**Pattern 87 promotion candidate.** The audit-column convention codified here is a candidate for promotion to `docs/guides/DEVELOPMENT_PATTERNS.md` Pattern 87 in a future V1.40+ revision: "Audit-column convention for canonical-tier tables — `created_at` always; `updated_at` only with a generic `set_updated_at()` BEFORE UPDATE trigger." Slot 87 is the next free Pattern slot post-V1.39 (slot 83 is taken by *Seed-Time FK Subquery Guard for Cross-Migration Lookups* per V1.37; slot 84 by *Two-Phase NOT VALID + VALIDATE CONSTRAINT* per V1.38; slot 85 reserved-for-future-promotion; slot 86 by *Living-Doc Freshness Markers* per V1.39). Promotion is deferred until at least one retrofit migration (#1074) has shipped to provide the canonical reference implementation.
...
- `canonical_market_links.canonical_market_id ON DELETE RESTRICT` (Cohort 3 design, v2.41 amendment + canonical_market_links DDL)
- `canonical_market_links.platform_market_id ON DELETE CASCADE` (Cohort 3 design, v2.41 amendment + canonical_market_links DDL)
- `canonical_match_log.link_id ON DELETE SET NULL` (Cohort 3 design, v2.41 amendment + canonical_match_log DDL — preserves audit history when a link is deleted)
...
ADR-119 continues with 0091-0097 (business-key cleanup + weather Phase 1).
...
- Epic #972 — Canonical Layer Foundation (this ADR and ADR-118 are its design anchors)
...
**END OF ARCHITECTURE DECISIONS V2.47**
