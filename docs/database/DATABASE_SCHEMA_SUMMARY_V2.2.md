...
<!-- RETIRED: V2.2 (alembic_head=0074, 2026-04-29) is superseded by V2.3 (alembic_head=0077, 2026-04-29) and subsequent versions through V2.4 (current canonical, alembic_head=0089, 2026-05-09). Kept for changelog history per Pattern 86 (DEVELOPMENT_PATTERNS V1.39+). For current schema state see DATABASE_SCHEMA_SUMMARY_V2.4.md. Banner added Slot 2 polish bundle, session 96 (Ripley P2-2); V2.4 alembic_head reference refreshed at Slot 5 V2.47 codification, session 99 (was 0086 → now 0089 reflecting Slot 4 lifecycle redistribution). -->

<!-- FRESHNESS: alembic_head=0074, verified=2026-04-29, tables=58, migrations=74, last_changelog_migration=0074 -->
<!--
Changelog from prior FRESHNESS marker (V2.1: alembic_head=0070, verified=2026-04-26, tables=51):
...
**Status:** Current — Alembic head: migration 0074 (chain: 0070 -> 0071 -> 0072 -> 0073 -> 0075 -> 0074)
**Replaces:** V2.1 (retained in same directory for changelog history; V2.0 + V1.16 also retained)

> **V2.2 Increment:** Brings the schema reference current with `alembic_version`
> after Cohort 3 closed in session 82.  Adds the seven Cohort-3 tables landed
> in Migrations 0071/0072/0073/0074/0075 (5 canonical_* tables + 2 Pattern 81
> lookups), refreshes the migration catalog (0071-0075), enumerates the SSOT
> Python vocabularies in `src/precog/database/constants.py` per Pattern 73,
> updates the Quick Reference counts (51 -> 58 tables, 69 -> 74 migrations,
> 52 -> 54 JSONB columns, 4 -> 6 Pattern 81 instances), corrects the freshness
> marker to `alembic_head=0074` (chain head, not the lexically-largest filename
> 0075), and marks Cohort 3 SHIPPED in the cohort roadmap.  Same structural
> shape as V2.1 — V2.2 is a faithful revision, not a re-architecture.
...
| Core Tables | 58 |
| Views | 19 |
| Migrations | 74 files (slots 0001-0075, slot 0029 skipped); chain head 0074 (non-monotonic) |
...
**Unchanged from V2.1.**  The eight canonical_* tables in Migrations 0067-0069
...
canonical identity layer per ADR-118 V2.38 + V2.39 + V2.40.  Full column
lists, CONSTRAINT TRIGGER details, and forward-pointer comments are in
V2.1 § G.

The tables Cohort 3 builds atop:
...
in § A) as link targets.  Cohort 3 closed in session 82 (PR #1094); Cohorts
4-9 will build the dim/fact split, event-state machine, views/CRUD/resolver,
seeders, business-key cleanup, and weather Phase 1 atop this foundation.
...
74 migration files cover slots 0001-0075 (slot 0029 intentionally skipped —
`portfolio_fills` design abandoned pre-baseline; chain head 0074 because slot
0075 ships ahead of slot 0074 per the non-monotonic design).  Cohort 3 additions
...
| **0074** | **Cohort 3 slot 5 — `canonical_match_overrides` + `canonical_match_reviews` (CLOSES Cohort 3)** | 2 new tables + 6 indexes + 5 CHECK constraints (incl. polarity-pairing three-valued-logic-safe form) + 1 UNIQUE constraint; chain head |

All migrations have `downgrade()` functions.  Slot 0029 intentionally skipped
(`portfolio_fills` eliminated).  **Non-monotonic chain head note:** slot 0074
is the alembic head despite slot 0075 being numerically larger.  The chain is
`0070 -> 0071 -> 0072 -> 0073 -> 0075 -> 0074` (per `down_revision` declarations).
Slot 0075 ships ahead of slot 0074 because the two are independent (no FK
dependencies between `observation_source` and the matching ledger).  A reader
scanning by lexical filename order will see 0075 numerically last — trust the
`down_revision` chain, not the filenames.
...
| **3** | **0071-0075 (SHIPPED s79-82)** | **matching infrastructure: `match_algorithm` (lookup) + `canonical_market_links` + `canonical_event_links` (link tables) + `canonical_match_log` (audit ledger) + `observation_source` (lookup) + `canonical_match_overrides` + `canonical_match_reviews` (operator state machine). 5-slot LOCK per session-78 council adjudication (Holden's FK-ordering argument: lookup ships first standalone). Cohort 3 CLOSES with slot 0074.** |
| 4 | 0076-0079 (queued) | dim/fact split refinement + slot 0076+ migration retrofits (BEFORE UPDATE trigger generic `set_updated_at()` per V2.42 sub-amendment A / #1074; canonical_events FK ON DELETE SET NULL retrofit per V2.42 sub-amendment B / #1075) |
| 5 | 0080-0083 (queued) | event-state machine + canonical_event_phase_log + matcher resolver code |
| 6 | 0084-0085 (queued) | views / CRUD / resolver |
| 7 | 0086-0091 (queued) | seeding (canonical_events 1:1 from games) + #937 |
| 8 | 0092-0095 (queued) | business-key cleanup (per ADR-119) |
| 9 | 0096-0098 (queued) | weather Phase 1 (per ADR-119) |
...
near the top of the file.  V2.2 example:
