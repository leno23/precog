...
- Origin: Session 70 Task 5 Round 2A (Isidore postgres-dev MCP audit of 17 `*_key` columns). Canonicalized via ADR-119 Part 1. Prevented naive "kill all formatted-PK decoration" lint from sweeping up legitimate SCD-2 surrogates during ADR-119's Part 1 migration cohort.
- Pairs with ADR-119 (decision context) and ADR-117 (tier model — Pattern 80 clarifies the Tier-2 SCD-2 exception).
**Changes in V1.34:**
...
- **Added Pattern 76: Session vs Trigger Nomenclature (ALWAYS for Prose Referencing Session or Trigger Numbers)**
- **Added Pattern 77: PM Triage Beats Mechanical Thresholds for Soft Gates (ALWAYS for Tier B / Non-Load-Bearing Audits)**
- **Added Pattern 78: Two-Gate Audit Discipline — Bug Presence + Fix Validity (ALWAYS for Retrospective Claude-Review Audits)**
- **Extended Pattern 43:** Added "Audit Checklist — Grep Scope" subsection with the 4-grep checklist (function-name, `fetchone.side_effect`, `execute.call_count`, `call_args_list` index). Source: Session 60 C2c Migration 0062 incident — 3 agents signed off, pre-push caught 18 stale mocks across unit + chaos tiers.
- **Extended Pattern 73:** Added "Systematic Compliance — Detection & Prevention" subsection covering detection heuristic, consolidate-or-point rule, and the recursive case (Pattern 73 applies to Pattern 73 itself). Source: Session 66 three-violation cluster (claude-review parse rule, issue slotting, Pipeline Completeness Gate invocation).
- Origin: Session 67 S80 first periodic sweep — 5 new patterns + 2 extensions promoted from the `feedback_*.md` queue
**Changes in V1.32:**
...
### Audit Checklist — Grep Scope (V1.33 extension)

When a Tier 2 CRUD PR changes a function's **query shape** (columns added to `RETURNING`, `SELECT` list in a `FOR UPDATE` lock, `ON CONFLICT DO UPDATE ... RETURNING`), Pattern 43 compliance requires greps across the **whole test suite**, not just files named after the function. Mocks of `cursor.fetchone()` and `cursor.execute()` live anywhere that patches `get_cursor` — they can sit in test files with no obvious name relationship to the CRUD function.

Run all four greps. Each catches a different failure mode:

```bash
# Scope 1 — Direct consumers (obvious; name-matched tests)
grep -rn "<function_name>" tests/ --include="*.py"

# Scope 2 — Mock-shape consumers (the easy miss)
#   Hits tests that will raise KeyError / StopIteration when the shape changes.
grep -rn "fetchone\.side_effect\|fetchone\.return_value" tests/ --include="*.py" | \
    grep -E "id['\"]|returning" -i

# Scope 3 — execute.call_count assertions (indirect break)
#   An extra UPDATE from a two-step pattern invalidates these.
grep -rn "execute\.call_count == [0-9]" tests/ --include="*.py"

# Scope 4 — call_args_list index assertions (index-shift break)
#   Adding a SQL call shifts every subsequent positional index.
grep -rn "call_args_list\[[0-9]+\]" tests/ --include="*.py"
```

### The Session 60 incident (canonical example)

**Setup.** Migration 0062 (C2c business keys) modified five CRUD functions:
- `create_market`, `create_event`, `create_game_state` — two-step INSERT (3 execute calls: INSERT → UPDATE → snapshot INSERT)
- `upsert_game_state` — `FOR UPDATE` lock-query SELECT list expanded to include `game_state_key`
- `get_or_create_game` — `ON CONFLICT DO UPDATE RETURNING` now includes `game_key`

**What the three agents caught.**
- **Samwise (Builder)** ran `tests/integration/ + tests/property/ + tests/race/` → 1349 pass, 0 fail. Did NOT run `tests/chaos/` or `tests/unit/`.
- **Glokta (Reviewer)** grepped `INSERT INTO markets|events|games` → found 14 raw-SQL fixture files. Fixed.
- **Ripley (Sentinel)** Priority 2 checked CRUD-semantic equivalence at **return-value level**. Did NOT grep `fetchone.side_effect`.

**What pre-push caught (18 failures the three agents missed).**
- 11 failures in `tests/chaos/test_crud_operations_chaos.py` — stale `fetchone.side_effect` lists missing the new lock-query slot
- 7 failures in `tests/unit/database/test_crud_operations_unit.py` — stale `fetchone.return_value` dicts missing `game_key` / `game_state_key`, plus three tests with `execute.call_count == 2` assertions and `call_args_list[1]` indices that became invalid when the two-step UPDATE landed at index 1

**Root cause of the miss.** Each agent had a narrow grep scope: Samwise excluded two tiers, Glokta's grep was `INSERT INTO <table>` (no mock-dict coverage), Ripley's equivalence check was `result == N`-level (no descent to cursor-mock consistency). The failures are all Pattern 43 violations — the "mock dict must match the real query's SELECT columns" rule only surfaces if you **grep the mocks**, not just run the tests.

**Cost math.** Running the 4-grep checklist is ~2 minutes. Hitting pre-push failures on a 6-minute push is ~18 minutes round-trip plus context-switch cost. The checklist wins ~10x on a single miss.

### Integration with agent briefs

- **Builder (Samwise) dispatch on CRUD query-shape changes:** "After your implementation, run the full `scripts/pre-push-validation.sh` locally before reporting done. Pre-push runs stress / chaos / race / unit in parallel — each tier has its own stale-mock risk surface."
- **Reviewer (Glokta) dispatch on T9 migrations:** "Pattern 43 audit has 4 greps: (1) `<function_name>`, (2) `fetchone.side_effect/return_value`, (3) `execute.call_count == N`, (4) `call_args_list[N]`. Do all 4 — do not rely on scope 1 alone."
- **Sentinel (Ripley) dispatch after remediation:** "When the PR changes a CRUD function's query shape, your Priority 2 CRUD-semantic-equivalence check must descend to cursor-level: grep `fetchone.side_effect` and `fetchone.return_value` for the affected function's mock consumers. Return-value equivalence ≠ mock-dict equivalence."
...
- Pattern 58: Triple-Reviewer Convergence — reviewer-stack design principle; this extension documents where convergence *fails* when greps share a blind spot
- Session 33: Glokta review finding #76, deployment log insight #74 (original pattern)
- Session 60: C2c Migration 0062 incident — 18 stale mocks surfaced at pre-push (V1.33 extension source); see `feedback_mock_fidelity_pattern43_grep_scope.md`
...
### Systematic Compliance — Detection & Prevention (V1.33 extension)

Pattern 73 started as a markdown-refactor idiom (preserve heading, replace body with pointer). Session 66 exposed a broader framing: **Single Source of Truth applies at every level of the project.** Any rule, value, formula, SQL idiom, validation check, docstring assertion, or process statement that appears in more than one location will drift unless exactly one location is canonical and the others are pointers. This is the default outcome, not an edge case.

#### The three-violation-in-one-session signal

Session 66 surfaced three Pattern 73 violations with identical failure shape:

1. **Claude-review parse rule.** Canonical rule lived in the S68 Decay Panel row; session-start, session-end, and Step 7a invocation points didn't reference it. Session 65 audit substituted a narrow `### Bug:` filter that was never written anywhere explicitly — silent drift between "rule as stated" and "rule as applied."
2. **Issue slotting.** Protocol said "kanban board is SSOT" but the issue-creation workflow never referenced this. PM substituted labels + session ACTUAL + MEMORY Active Priorities as de facto tracking; kanban board became the documented-but-unused source of truth.
3. **Pipeline Completeness Gate (S76).** Rule exists in the Decay Panel, but the actual push path doesn't force verification. User's question "did Samwise's code undergo review?" caught a near-miss on a bundle PR.

Three instances in a single session is **systemic signal** — the protocols have rules but weak binding between "rule statement" and "invocation point that should fire the rule."

#### Detection heuristic

Ask this at every authorship or audit moment:

> *"If I edit this rule in one place, what's the probability all other copies get updated atomically?"*

If the answer is anything below 100%, you have a Pattern 73 violation. Drift is not a question of *if* but *when*.

Concrete signals that a Pattern 73 violation is forming:
- A rule, formula, or check appears in two or more `.md` files without a pointer linking them
- A docstring parrots a formula from the code body (e.g., `crud_analytics.py:432-433` docstring restating the view's P&L arithmetic verbatim)
- A constant is redeclared in multiple modules instead of imported from a single `constants.py`
- A protocol step says "do X as defined in Y" but the reader has to guess what Y is
- Two migrations duplicate the same SCD idiom instead of extracting a helper

#### Prevention — the consolidate-or-point rule

At protocol/doc write time: if you catch yourself writing "the rule for X is Y" in two places, **STOP**. Consolidate to one canonical location with a clear anchor. Every other location becomes a pointer:

```markdown
<!-- CANONICAL location (the one place the rule is stated) -->
## § Step 7a — Claude-review parse rule
Apply this regex to every merged PR: `^### (Section|Bug|Issue|...)`. Severity markers count.
Skip nothing under `### Minor notes` — session 65 audit missed 12+ findings by filtering to `### Bug:` only.

<!-- POINTER locations (everywhere else the rule is referenced) -->
Session-start checklist step 5a: "apply the canonical parse rule from [§ Step 7a](#step-7a)."
Session-end checklist step 6b: "re-apply [§ Step 7a](#step-7a) for any PR merged since last session."
```

At code write time, same principle:
- Constants live in `db/constants.py`; consumers import
- SQL idioms extract to `crud_shared.py` helpers
- Validation rules have one implementation with a clear name; callers import

#### The recursive case — Pattern 73 applies to Pattern 73
...
During audits (S80 periodic, S81 retrospective, code review), flag Pattern 73 violations as P2 with a specific remediation: "consolidate A and B, make B a pointer." Example issues from session 66: #927 (Issue Slotting Checklist META), #929 (Pattern 73 compliance audit META).

#### Forward application

- Every new protocol section — cross-check against existing sections before writing
- Every new feedback memory — is it a pointer to existing canonical, or genuinely new?
- Every new CRUD function — does it duplicate an idiom that should be a helper?
- Every new migration — does its SQL duplicate a pattern five other migrations have?
- Every new test fixture — does it belong in `conftest.py` as a shared fixture?
- S81 re-audit (retrospective claude-review) includes Pattern 73 detection in its criteria
- S80 periodic pattern-promotion sweep surfaces Pattern 73 compliance drift

### Cross-References

- CLAUDE.md Critical Pattern #8 — "Single Source of Truth" short-form invocation; points here for mechanics
- `protocols.md § Step 7a` — canonical claude-review parse rule (the first violation collapsed in session 66)
- Pattern 70 (Convergent-Reviewer Signal Rule) — independent convergence IS a Pattern 73–safe design, because both reviewers derive findings from the same source without duplicating the rule text
- `feedback_pattern_73_recursive_application.md` — full narrative of the session 66 three-violation cluster
- Session 66 ACTUAL — contemporaneous observation context

---

## Pattern 74: Rename-Over-Rewrite for Audit Coverage Gaps (ALWAYS for Audit-Driven Test-Coverage Work)

**Severity:** MEDIUM — closing an audit gap by rewriting tests that already exist (under a different filename) creates duplicate-maintenance debt and burns a full Builder pipeline on a 1-minute `git mv`.

### Problem / Trigger

Test-type coverage audits (`scripts/audit_test_type_coverage.py`) discover tests via a filename glob (e.g., `test_*<module_name>*.py`). When a module's tests were written under a domain-named convention that predates the module's current name — or predates the audit — the audit reports a coverage gap that **isn't real**: coverage exists, just under a filename the glob doesn't match.

The mechanical response is to dispatch a Builder (Samwise) to write new tests. That's the wrong response. The right response is `git mv`.

### The Pattern / Rule

Before dispatching a Builder to close a test-coverage gap surfaced by an audit:

1. **Step 0 of every audit-gap-closure dispatch — grep for existing coverage.** Budget: ~2 minutes.

   ```bash
   # Does existing test coverage exist under a non-matching filename?
   grep -l "from precog.database import <module>" tests/
   grep -l "import <module>" tests/
   ls tests/unit/database/ | grep -i <keywords-from-module-name>
   ```

2. **If siblings exist and look healthy — rename.** `git mv` the sibling files to match the audit's glob. Run pytest on the renamed files to confirm nothing broke. Verify the audit now passes. Zero new test code.

3. **If siblings exist but are thin / stale — rewrite, but with context.** Dispatch Builder, but provide the existing file paths as "reference implementations you're replacing or extending" rather than starting from scratch.

4. **Only if no siblings exist — full Builder pipeline.** Samwise + Reviewer + Sentinel, per the usual audit-gap-closure flow.

5. **File a follow-up hygiene issue when renaming.** The renamed siblings likely predate current pattern hygiene (Pattern 43 mock fidelity, Pattern 45 None-short-circuit, SQL+params pinning). Opportunistic cleanup when the file is next touched. See #890 as the template.

### Wrong

```bash
# Audit reports: test_*crud_ledger*.py — no tests found. Gap flagged.
# Response: dispatch Samwise to write 645 LoC of new tests for crud_ledger.
# Result: pass the audit, but now duplicating 1,318 LoC / 87 tests already present
#         in tests/unit/database/test_account_ledger_crud.py + test_temporal_alignment_crud.py + test_market_trades_crud.py.
```
...
# Audit reports: test_*crud_ledger*.py — no tests found. Gap flagged.
# Step 0: grep first.
$ ls tests/unit/database/ | grep -iE "account_ledger|temporal_alignment|market_trades"
test_account_ledger_crud.py
test_temporal_alignment_crud.py
test_market_trades_crud.py

# Step 1: verify they cover crud_ledger's surface.
$ grep -l "from precog.database.crud_ledger import" tests/
tests/unit/database/test_account_ledger_crud.py
tests/unit/database/test_temporal_alignment_crud.py
tests/unit/database/test_market_trades_crud.py

# Step 2: rename to match the audit glob.
$ git mv tests/unit/database/test_account_ledger_crud.py      tests/unit/database/test_crud_ledger_account.py
$ git mv tests/unit/database/test_temporal_alignment_crud.py   tests/unit/database/test_crud_ledger_temporal.py
$ git mv tests/unit/database/test_market_trades_crud.py        tests/unit/database/test_crud_ledger_trades.py

# Step 3: confirm tests still pass, audit now clean.
$ pytest tests/unit/database/test_crud_ledger_*.py -q
$ python scripts/audit_test_type_coverage.py
```

Zero new test code. 87 existing tests preserved. Audit gap closed in under a minute.

### Budget Impact

Session 63 (#887 CRUD burn-down) had 8 modules with audit gaps. Per-module cost:
- **Full Builder pipeline:** ~30–40 min (dispatch + review + sentinel + merge)
- **Grep + rename:** ~5–10 min when siblings exist

Estimated savings: 4–6 hours across the remaining burn-down — if Step 0 is applied first.

### When NOT to Apply

- Siblings exist but are **stale or thin** (pre-Pattern-43, no mock fidelity, missing error paths) → rewrite, don't rename. The sibling file is a starting reference, not the final answer.
- Module genuinely has no test coverage → full Builder pipeline.
- Rename would collide with an existing matching file → either consolidate or rewrite.

### Source

- Session 63 PR #889 — the first rename-path burn-down (3 sibling files renamed, audit gap closed, zero new code)
- Session 63 PR #888 — the rewrite-path pilot (crud_lookups), used when no siblings exist
- `scripts/audit_test_type_coverage.py:372` — the glob that drives this
- Umbrella #887 — the CRUD burn-down that benefits from this rule
- `feedback_rename_over_rewrite_for_audit_gaps.md` — canonical origin memo

---

## Pattern 75: Verify Active Priorities Before Dispatch (ALWAYS for MEMORY.md-Driven Agent Work)

**Severity:** MEDIUM — dispatching agents against an "Active Priority" that's already been fixed burns 20–30 min per false positive and seeds false confidence in the priority list.

### Problem / Trigger

MEMORY.md maintains a curated "Active Priorities" section as the next-session anchor. This list decays silently when:

- A fixer PR merges without `Closes #NNN` in the body
- GitHub issue is closed manually later (or never) while MEMORY.md keeps citing it
- The fix-commit comment block references the issue number inline, but nobody regrep'd during the decay window
- Multi-session gap — priorities written in session N can be stale by session N+5 without notice

Session 64 discovered that the TOP TWO "HIGH PRIORITY" Active Priorities (#662 and #622) were BOTH already fully fixed. #662 by commit `af73928` (PR #688), #622 by commit `309b3f8` + migration 0051 (PR #690). Both GitHub issues were already CLOSED. Only MEMORY.md was stale — for ~20 sessions.

### The Pattern / Rule

Before dispatching any agent against an item from MEMORY.md's "Active Priorities" section, **run live-state verification**. Do not trust the list as a live queue.

Minimum checks, per priority:

1. **GitHub state.** `gh issue view N --json state` — is the issue actually open?
2. **Schema assertions.** If the priority cites a column/index/constraint, use the `postgres-dev` MCP (`describe_table`, `query_database`) to verify current schema.
3. **Code assertions.** If the priority cites a function's behavior, `grep` the current source and read the current body. The fix comment often literally cites the issue number — a 10-second grep surfaces it.
4. **Bug assertions.** If the priority cites a symptom, grep for the symptom's marker (issue number in a comment, inline TODO, etc.).

If the check shows the item is **already done**:
- Scrub it from MEMORY.md's Active Priorities block
- Close the GitHub issue if still open
- Record the verification in the session ACTUAL (so the next session doesn't re-verify)
- **Do this BEFORE considering any agent dispatch.**

### Wrong

```markdown
# MEMORY.md Active Priorities (written session 44):
- **#662 — update_position_price drops execution_environment** — HIGH PRIORITY
- **#622 — account_balance missing execution_environment column** — HIGH PRIORITY

# Session 64 PM reads MEMORY.md → sees top-two priorities → plans Mulder+Holden dispatch for design pass.
# Result: 60+ min of agent work on bugs that were fixed ~20 sessions ago. Both GitHub issues already closed.
```

### Right

```bash
# Session-start, BEFORE any dispatch planning.
$ gh issue view 662 --json state,closedAt
{"state": "CLOSED", "closedAt": "2026-02-14T..."}

$ gh issue view 622 --json state,closedAt
{"state": "CLOSED", "closedAt": "2026-02-18T..."}

# 10-second follow-up grep confirms fix is IN the code.
$ grep -n "Issue #662" src/precog/database/
src/precog/database/crud_positions.py:847:    # Issue #662: Marvin sentinel pass found that omitting
                                           #   this column silently flipped non-'live' positions...

# Scrub from MEMORY.md. Do not dispatch.
```

### Prevention Layer — Write `Closes #NNN` in the PR body

The root cause of Active Priority decay is fixer PRs that don't include `Closes #NNN` in the **body**. GitHub auto-closes on merge for body references (not commit-message references). This is a one-line fix at PR-creation time that prevents months of downstream decay.

### When to Escalate

If this rule catches stale priorities ≥2 times in a row, promote the check from "ad-hoc session-start discipline" to an explicit step in the session-start protocol (`protocols.md § Step 5`). Repeated fires = systemic signal that the Active Priorities list needs structural hardening, not just manual vigilance.

### Source

- Session 64 ACTUAL — the #662 + #622 false-positive catch that surfaced the rule
- `protocols.md § Step 5` — current session-start gate (extends here for Active Priorities specifically)
- `feedback_memory_active_priority_decay.md` — canonical origin memo
- Pattern 78 (Two-Gate Audit Discipline) — adjacent rule for retrospective claude-review audits

---

## Pattern 76: Session vs Trigger Nomenclature (ALWAYS for Prose Referencing Session or Trigger Numbers)

**Severity:** LOW — ambiguity in prose that only surfaces when memory files are read fresh in future sessions, but compounds over dozens of memory files.

### Problem / Trigger

The project has two parallel integer-indexed namespaces:

- **Sessions.** 5-hour quota windows, numbered 1, 2, ..., 66, 67, ...
- **S-triggers.** Periodic and on-change triggers in `roster_triggers.md`: `S1`, `S60`, `S68`, `S73`, `S74`, `S80`, `S81`, ...

When sessions are written `S63` as shorthand for "session 63", sentences like *"the S68 fire in S63 reconstructed the S77 ANNOUNCE record for S62"* become ambiguous: is `S63` a trigger that doesn't exist yet, or a session number? The reader has to disambiguate by context — and context decays across sessions.

### The Pattern / Rule

- **Sessions:** write `session 62`, `session 63`, `sessions 64–68`, `this session`. Lowercase. Integer. Spelled out.
- **S-triggers:** write `S68` (Claude Review audit), `S73` (VCR test mocking), `S74` (assumption decay audit). Uppercase `S` + number, per `roster_triggers.md`.
- **Rule:** never write `S63` meaning "session 63". If you see `S63` in new text, assume it's a typo for a trigger that doesn't exist and correct it.

### Wrong

```markdown
S62's retrofit (S64 trigger fire) caught the S76 near-miss.
```

Ambiguous: are S62, S64, S76 all triggers? Is S62 a session number and S64 a trigger? The reader has to check `roster_triggers.md` against valid-trigger IDs to disambiguate — and when a trigger is added later, past prose can become retroactively ambiguous.

### Right

```markdown
Session 62's retrofit (S64 trigger fire) caught the S76 near-miss.
```

One namespace per token shape. Sessions are spelled out lowercase; triggers keep the `S`-prefix. No disambiguation needed.

### Filename Convention Unchanged

Files like `session_62_actual.md`, `session_history_40_49.md` keep their existing naming — the `session_` prefix already disambiguates. The rule applies to **prose** inside memory files, protocol sections, PR descriptions, and commit messages.

### Retroactive Cleanup

Existing memory files have mixed usage. **Do not do a mass rename.** The convention applies going forward in new text. Existing files stay as-is unless they're being edited for another reason — opportunistic cleanup only.

### Source

- Session 62 — the first session where both `S62` and "session 62" appeared in the same paragraph and triggered the disambiguation question
- `roster_triggers.md` — canonical S-trigger namespace owner
- `feedback_nomenclature_sessions_vs_triggers.md` — canonical origin memo

---

## Pattern 77: PM Triage Beats Mechanical Thresholds for Soft Gates (ALWAYS for Tier B / Non-Load-Bearing Audits)

**Severity:** MEDIUM — mechanical thresholds on soft gates add friction without adding accuracy, and drift silently when the underlying unit is under-specified.

### Problem / Trigger

The original S62 Tier B Module Health Check trigger fired when a module received "3+ changes in a session." Session 65 exposed three weaknesses in that formulation:

1. **"Change" unit was undefined.** Commit? PR? Edit call? Logical change across three Samwise rounds? Session 62's 0064 rebuild (3 Samwise rounds → 1 PR) and session 65's Gap A (1 PR → 1 commit) both count as "1 change" by PR but "3 changes" by iteration. Opposite verdicts from the same threshold.
2. **No risk weighting.** A single-line change in `trading/positions.py` (money-path) arguably deserves MORE scrutiny than 10 changes in `cli/config.py` (ergonomic). Tier 1 Override already embeds risk weighting for Tier 2 escalation; S62 didn't mirror that insight.
3. **No rolling window.** 2 changes this session + 2 last session = 4 cumulative on the same module → 0 S62 fires. Steady-drift modules are exactly the modules S62 SHOULD fire on most.

### The Pattern / Rule

**Mechanical thresholds are appropriate for hard gates where missing the fire is expensive** (S68: missed claude-review bug ships to main; S60: new production module without P41 ships unmonitored). For **soft gates** — where the failure mode on both sides is cheap — **PM triage beats counting**.

Soft-gate failure-mode check:

- **False negative** (audit should fire, doesn't): next session catches the issue; no load-bearing cost.
- **False positive** (audit fires, module didn't need it): 10-minute PM spot-check; cheap in absolute terms.

When both failure modes are cheap, replace the threshold with a per-session yes/no triage:

> *"Did this module get real work this session — logic / data flow / contract — or cosmetic-only?"*

- **Yes** → run the audit (for S62, that's the 7-item P41 spot-check)
- **N/A-cosmetic** → record as skipped in the verdict table

Triage hints:
- **Lean YES:** money / SCD / credentials / scheduling domain, production service, new logic, Tier 2 pipeline PR
- **Lean N/A-skip:** cosmetic-only, rename refactor, test-file-only with src untouched, non-production scripts
- **When ambiguous:** default to "worth checking." A 10-minute spot-check is cheap; skipping a module that turned out to need checking is more expensive.

Record verdict in the session-end verdict table: `module_name — P41 spot-check: OK / issues-noted` OR `module_name — cosmetic/N/A`.

### Wrong

```markdown
# Mechanical threshold (old S62):
# Fire audit if >= 3 "changes" to the module this session.

# Session 62: 0064 rebuild — 3 Samwise rounds, 1 merged PR. Is that 1 change or 3?
# → Verdict depends on undefined "change" unit. Skip or fire, the threshold hasn't helped.
```

### Right

```markdown
# PM triage (session 65+ S62):
# At session end, walk through each module touched this session.
# For each: yes/no on "real work vs cosmetic-only."

- `trading/positions.py`   — yes (money-path, SCD write) → P41 spot-check: OK
- `cli/scheduler_commands.py` — no (1 rename, no logic)    → cosmetic/N/A
- `analytics/model_manager.py` — yes (new method)           → P41 spot-check: issues-noted (alert config gap, follow-up filed)
```

### When This Pattern Applies

- **Soft audits** where both failure modes are cheap: S62 Tier B Module Health, S67 doc freshness, S80 pattern-promotion sweep
- **Judgment-beats-counting** contexts: risk-weighted dispatch decisions, scope shrinks, reviewer-selection

### When NOT to Apply (keep the mechanical threshold)

- **Hard gates** where missing the fire ships bad code to main: S68 (claude-review audit on merged PRs), S60 (new production module without P41), S76 (Pipeline Completeness Gate before push)
- **Legal / compliance / security contexts** where "missed but cheap" is wrong — the cost of a miss is asymmetric
- **Audits where PM can't reliably triage** (no domain knowledge, no session-scope visibility)

### Files Updated Session 65

- `roster_triggers.md` S62 row — cosmetic-only triggers explicitly listed under "NOT Triggered For"
- `three_tier_quality_model.md` Tier B section — triage hints + spot-check separated
- `checklist_session_end.md` Step 6c — triage-first structure

### Source

- Session 65 user question: *"what is S62 and is 3+ module changes the appropriate trigger?"* — exposed the threshold as both under-specified and a poor match for Tier B's lightweight purpose
- `feedback_s62_pm_triage_replaces_threshold.md` — canonical origin memo
- S62, S67 triggers in `roster_triggers.md` — the soft-gate audits this pattern applies to

---

## Pattern 78: Two-Gate Audit Discipline — Bug Presence + Fix Validity (ALWAYS for Retrospective Claude-Review Audits)

**Severity:** MEDIUM — one-gate audits produce plausible-looking findings that either waste Builder cycles (Builder refuses) or ship broken fixes (PM-direct mechanical application).

### Problem / Trigger

Retrospective claude-review audits (session 65's pre-S68 backlog audit, session 66's #910 fallout) age. A finding written in March against a pre-refactor codebase can:

- Still *look* present in current code via grep (the SQL really is missing the filter the reviewer cited)
- Fail at runtime because the surrounding context (schema, architecture, sibling functions) shifted post-review to eliminate the pattern entirely

Session 65's pre-S68 audit ran "Gate A" only — does the bug still appear present? — and filed #910 against `crud_markets.py:417-419` for missing `row_current_ind = TRUE`. Samwise refused to apply the fix after MCP-verifying that the `markets` table has never had `row_current_ind` since Migration 0021 (markets_split, March 2026). The `markets` table is a dimension, not a fact. The reviewer's prescribed fix would have caused a runtime error. **#910 was a Gate-A pass, Gate-B fail — a false positive that cost ~30 min of agent cycle time to discover.**

### The Pattern / Rule

Every retrospective claude-review finding must pass **BOTH**:

#### Gate A — Coverage (bug presence)

Does the bug still *appear* present in current code? Standard grep / MCP / read-the-file verification.

```bash
# E.g., "review cited missing row_current_ind filter at crud_markets.py:417-419"
grep -n "row_current_ind" src/precog/database/crud_markets.py:417-419  # still missing?
```

#### Gate B — Fix validity (is the proposed fix still correct?)

Given the CURRENT schema, patterns, and architecture, is the proposed fix still correct? Or has the surrounding context shifted in a way that makes the fix nonsensical?

```bash
# Gate B questions:
# 1. Do the columns referenced in the fix still exist?
mcp-postgres describe_table markets
#    → Session 66 check: markets has no row_current_ind column. Never has, since Migration 0021.

# 2. Do the sibling functions the review cited as "this is how it's done elsewhere" still exist?
grep -n "def get_current_market_by_ticker\|def list_open_markets" src/precog/database/crud_markets.py
#    → Session 66 check: neither function exists. Review context is pre-0021.

# 3. Does the fix's mechanism still apply under current architecture?
#    → Session 66 check: markets is now a dimension table; SCD2 moved to market_snapshots.
#      The "missing row_current_ind" is structural, not a bug.
```

**File an issue only if BOTH gates pass.** If Gate B fails, file a different kind of issue: *"claude-review finding on PR #N is audit-stale because schema/patterns diverged post-review."* The issue is about audit methodology, not about a live bug.

### Report rates separately

A retrospective audit has two distinct quality measures. Report both — tracking only one hides methodology drift.

- **Gate A rate** = (findings where bug still present) / (findings examined)
- **Gate B rate** = (findings where fix still valid) / (findings that passed Gate A)

Session 65 pre-S68 audit: Gate A = 89% (8/9 findings were live bugs). Gate B was not measured. Session 66's #910 investigation retroactively computed Gate B = 89% (8/9 findings had valid fixes) — the one divergent case was #910 itself. The coincidence of matching rates is informative precisely *because* they measure different things.

### Wrong

```markdown
# Session 65 pre-S68 audit methodology:
# For each claude-review finding in pre-#585 PRs:
#   1. Grep for the cited bug in current code.
#   2. If present, file an issue.
# Result: 9 issues filed (#909-#917). 1 (#910) was a false positive caught by Builder refusal.
```

### Right

```markdown
# Session 66+ methodology:
# For each claude-review finding in pre-#N PRs:
#   Gate A — Does the bug still appear present? (grep current code)
#   Gate B — MCP-verify current schema. Grep cited sibling functions.
#            Verify fix's mechanism still applies under current architecture.
#   File issue only if BOTH gates pass.
#   If Gate A passes + Gate B fails, file a methodology issue:
#     "claude-review finding on PR #N is audit-stale — schema/patterns diverged post-review"
# Report: Gate A rate + Gate B rate separately.
```

### The Session 66 Canonical Example (#910)

- **#910 body claimed:** `get_current_market_by_ticker` (at `:187`) and `list_open_markets` (at `:254`) filter correctly; `crud_markets.py:417-419` should do the same.
- **Current reality:** neither cited function exists in `crud_markets.py`. Functions that exist: `count_open_markets` (:365) + `count_open_markets_by_subcategory` (:392). All 10 `row_current_ind` refs in the file target `ms.row_current_ind` (the `market_snapshots` fact table), not `m.row_current_ind`.
- **Diagnosis:** the claude-review on PR #562 was written against a pre-0021 codebase that HAD the cited sibling functions. Migration 0021 (markets_split, committed March 2026) restructured markets into dimension + market_snapshots (fact), eliminating the SCD pattern from `markets` entirely. The review's prescribed fix became runtime-invalid at that point.
- **Outcome:** #910 closed INVALID. Samwise Builder discipline caught the schema drift and refused to apply the fix. The meta-issue feeding this pattern was filed as the audit-methodology improvement.

### When This Pattern Applies

- Every pre-S68 backlog audit (session 65's scope, session 67+'s expansions)
- S81 retrospective claude-review re-audits (every sweep must include Gate B)
- Pre-#800 S68 verification audit (#924) — MUST include Gate B verification
- Any future retrospective claude-review audit

### When NOT to Apply

- **Forward-looking** claude-review audits (S68 on this-session-merged PRs) — the review is current, surrounding context hasn't had time to drift, Gate B is trivially satisfied
- Audits of non-claude-review findings (Joe Chip adversarial reviews, Ripley sentinel passes) — different decay model
- Live-bug reports from production (no retrospective component)

### Source

- Session 66 #910 false positive — the trigger that surfaced the gap
- Session 66 Samwise Builder refusal — the discipline that caught the runtime-invalid fix
- `audit_pre_s68_backlog.md` — the session 65 audit that produced #910 (Gate A-only methodology)
...
## Pattern 80: SCD-2 Version-Stable Surrogate Identifiers (ALWAYS When Deciding Whether a `{PREFIX}-{id}` Business-Key Column Is Decoration or Load-Bearing)

**Severity:** HIGH — misclassifying a legitimate SCD-2 version-stable surrogate as "formatted-PK decoration" and deleting it destroys the only row-stable external reference to a logical entity whose row `id` changes on every version write. Misclassifying pure decoration as "might be SCD-2 stable" propagates `CAN-EVT-{id}` / `CAN-MKT-{id}` noise into every new canonical-layer table. The `row_current_ind` gate resolves both directions.

### Problem / Trigger

A subset of the schema's business-key columns follow the shape `{PREFIX}-{id}` (e.g., `GAM-4821`, `MKT-8932`, `POS-4821`). On surface inspection they all look like the same thing: a formatted projection of a SERIAL PK into a human-readable column. A naive "delete formatted-PK decoration" lint or audit sweeps them all up.

That lint is wrong. Three of these columns (`game_states.game_state_key`, `positions.position_key`, `edges.edge_key`) live on **SCD-2 tables** where the row `id` changes on every version write. The column supplies the row-stable *external* reference that `id` cannot: "position 4821" must resolve to the same logical position regardless of how many versions have been written. Three other columns (`games.game_key`, `markets.market_key`, `events.event_key`) live on tables that are **not SCD-2**; their `{PREFIX}-{id}` column adds no information beyond the PK itself.

The two classes look identical by grep; only the presence of SCD-2 versioning — and specifically the `row_current_ind` column that marks it — distinguishes them.

This was exactly the failure mode Isidore's Round 2A audit (session 70 Task 5) surfaced across 17 `*_key` columns. ADR-119 Part 1 canonicalized the rule. The lint rule below is the mechanical form.

### The Pattern / Rule

**Canonical definition lives in [ADR-119 Part 1](../foundation/ARCHITECTURE_DECISIONS.md#part-1--business-key-cleanup)** — the per-column classification table and the "SCD-2 version-stable surrogate pattern (documented exception)" subsection are authoritative. Per Pattern 73, this pattern does not restate the per-column decisions — it codifies the *gate that distinguishes the two classes*:
...
```python
# Naive "kill formatted-PK decoration" lint — sweeps up legitimate SCD-2
# surrogates along with actual decoration. Run against the live schema as
...
def find_formatted_pk_decoration(inspector, connection):
    ...
    for table in inspector.get_table_names():
        for col in inspector.get_columns(table):
            if col["name"].endswith("_key") and col["type"].python_type is str:
                # Does the column store a {PREFIX}-{id} projection?
                sample = connection.execute(
                    text(f"SELECT {col['name']} FROM {table} LIMIT 1")
                ).scalar()
                if sample and re.fullmatch(r"[A-Z]{3,4}-\d+", sample):
                    candidates.append((table, col["name"]))  # DELETE candidate
    return candidates
# Missing: the `row_current_ind` gate. False positives guaranteed.
```

```sql
-- An audit that blindly applies "formatted-PK decoration = delete":
-- >>> DELETE the SCD-2 surrogate `position_key` from `positions`
-- Effect: every external reference to "position 4821" now depends on the
-- row `id`, which changes on every SCD-2 write. Consumers of the
-- position_key stabilization contract break silently.
```
...
def find_formatted_pk_decoration(inspector, connection):
    ...
    for table in inspector.get_table_names():
        ...
        is_scd2 = SCD2_MARKER_COLUMN in col_names
        for col in inspector.get_columns(table):
            if not col["name"].endswith("_key"):
                continue
            sample = connection.execute(
                text(f"SELECT {col['name']} FROM {table} LIMIT 1")
            ).scalar()
            if sample and re.fullmatch(r"[A-Z]{3,4}-\d+", sample):
                if is_scd2:
                    # KEEP — version-stable surrogate required for external
                    # reference across SCD-2 version chain.
                    continue
                decoration.append((table, col["name"]))
    return decoration
```

```sql
-- Decision matrix applied per ADR-119 Part 1:
--   games.game_key           -- NOT SCD-2 (no row_current_ind) → DELETE
--   markets.market_key       -- NOT SCD-2                      → DELETE
--   events.event_key         -- NOT SCD-2                      → DELETE
--   game_states.game_state_key -- SCD-2 (row_current_ind present) → KEEP
--   positions.position_key     -- SCD-2                         → KEEP
--   edges.edge_key             -- SCD-2                         → KEEP
--
-- Canonical-layer commitment (ADR-119): new canonical_events /
-- canonical_markets tables will NOT introduce canonical_key / canonical_
-- market_key columns in {CAN-EVT-{id}} / {CAN-MKT-{id}} format. No SCD-2,
-- no legitimate need, no decoration propagation.
```

### Decision Checklist Before Adding, Keeping, or Removing a `{PREFIX}-{id}` Business-Key Column

1. **Does the owning table have `row_current_ind`?** Look at `inspector.get_columns(table)` or the migration's `op.create_table` column list.
2. **If YES — this is an SCD-2 table.** A `{PREFIX}-{id}` business-key column IS load-bearing: it supplies version-stable identity across the version chain. Keep it. Document the role in the migration docstring.
3. **If NO — this is not an SCD-2 table.** A `{PREFIX}-{id}` business-key column carries no information beyond the SERIAL PK. It is decoration. Delete it (or don't add it in the first place).
4. **Grep for existing external consumers.** Before deletion, confirm no code queries the column as a lookup predicate (outside test assertions). ADR-119 Part 1's blast-radius column documents the 144-refs / 34-file sweep method.
5. **Write the classification into the migration docstring.** "`game_key` is formatted-PK decoration — deletable per ADR-119 Part 1" OR "`position_key` is SCD-2 version-stable surrogate — REQUIRED for external-reference stability across version chain; protected by the `row_current_ind` lint gate." Future authors reading the migration see the rule at the site of the decision.

### When NOT to Apply (Future Rename Path)

Pattern 80's rule is *about the gate*, not about the column name. The SCD-2 version-stable surrogate columns will eventually be renamed to `version_stable_id` (per ADR-119 Part 1's "future Phase 2+ migration" note) for self-documenting naming. The rename preserves the gate semantics — they still live on SCD-2 tables, still serve version-stable identity, still survive an audit gated on `row_current_ind`. The gate is what persists; the column name is a cosmetic successor.

Until the rename ships, the `_key` suffix on these three columns remains the grep-target. The suffix alone is NOT enough — the `row_current_ind` gate is the discriminator.

### Pair With Pattern 79 (Tier-2 Nuance)

Pattern 79 (Three-Tier Identity Model) classifies `_key`-suffixed columns as *usually* Tier 2. Pattern 80 refines: within that Tier 2 set, SCD-2 version-stable surrogates are a documented subclass with their own architectural role. An automated lint applying Pattern 79 (no UNIQUE on Tier 3) will not false-positive on these columns. An automated lint auditing for formatted-PK decoration (Pattern 80's target) must gate on `row_current_ind` to avoid the false-negative that would delete them.

### Source

- Session 70 Task 5 Round 2A — Isidore postgres-dev MCP audit of 17 `*_key` columns; `feedback_bundle_adr_and_pattern_when_s80_on_change.md` precedent (ADR + Pattern bundled)
- ADR-119 Part 1 (Business-Key Cleanup) — decision context, per-column classification table, documented exception subsection, future-rename note
- ADR-117 (Three-Tier Identity Model) — the tier taxonomy this pattern refines within; Pattern 80 is the Tier-2 SCD-2 exception's operational gate
- ADR-116 (ODS Schema Conventions) — the `_key` suffix convention that lint rules grep for
...
**Canonical decision context lives in [ADR-118 v2.40](../foundation/ARCHITECTURE_DECISIONS.md) Cohort 1 carry-forward item 3** (single-phase justified by 0-row state on `canonical_events.lifecycle_phase` in Migration 0070 — the explicit precedent for the "When NOT to apply" carve-out below). Per Pattern 73, this pattern does not restate the per-migration justification — it codifies the *general rule*:
...
| 0079 | canonical_event_phase_log | `phase IN (...)` (mirrors 0070) | Single-phase | Shipped Cohort 4 slot 0079 (PR #1124, session 87) ahead of canonical_events seeder; audit-log co-populated by trigger from canonical_events.lifecycle_phase changes; 0-row state in dev at deploy made single-phase defensible. (Slot 0077 — separate work — landed canonical_events FK ON DELETE SET NULL retrofit per V2.42-B; the original "0077 planned" prediction was off-by-one against the V2.41 5-slot displacement.) |
...
- Added Pattern 74 (Rename-Over-Rewrite for Audit Coverage Gaps) — `git mv` closes audit globs at ~1% of a Builder pipeline's cost when sibling tests exist. Source: Session 63 PR #889 (`feedback_rename_over_rewrite_for_audit_gaps.md`).
- Added Pattern 75 (Verify Active Priorities Before Dispatch) — MEMORY.md "Active Priorities" decays; verify via `gh issue view` + MCP before dispatching agents. Source: Session 64 (`feedback_memory_active_priority_decay.md`) — top-two priorities found fixed 20+ sessions earlier.
- Added Pattern 76 (Session vs Trigger Nomenclature) — write "session 63" (integer); reserve "S63" for S-triggers in `roster_triggers.md`. Source: Session 62 (`feedback_nomenclature_sessions_vs_triggers.md`).
- Added Pattern 77 (PM Triage Beats Mechanical Thresholds for Soft Gates) — judgment beats counting when both failure modes are cheap. Source: Session 65 S62 trigger redefinition (`feedback_s62_pm_triage_replaces_threshold.md`).
- Added Pattern 78 (Two-Gate Audit Discipline — Bug Presence + Fix Validity) — every retrospective claude-review finding must pass Gate A (bug still present) AND Gate B (fix still valid under current schema/code). Source: Session 66 #910 false positive (`feedback_audit_fix_validity_gate.md`).
